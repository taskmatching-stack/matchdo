...
5. **關聯候選分組（同廠內）**（`manufacturer-materials.html`）：
   - **材料／配件 → 關聯主產品**：先依來源區（官方版型庫頁＝「官方版型」；個人廠商頁＝「廠商版型」），其下再依產品 **官方主分類**（`category_key`）分組，**不再**用廠商自建資料夾當主標題。
   - **主產品 → 關聯材料／配件**：來源區為「官方版型｜廠商版型｜供應商」（供應商＝`from_supplier_catalog` 匯入素材）；廠商／官方區內可再依「我的分類」細分。
   - 連結範圍仍僅 **同一 `manufacturer_id`**，不跨官方庫與個人廠商互相寫入。
