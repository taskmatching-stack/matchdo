...
## 1. 現況分析（Workspace 現態）
- 前端：Bootstrap / 靜態 HTML（public/admin/*、client/*）；專案詳情含工項標籤可編輯、預媒合推薦標籤可點擊加入、已媒合專家列表可刪除個別廠商。
- 後端：Node + Express（`server.js`）；**預媒合第一版已完成**（`POST /api/match/preview`：依工項+標籤篩選、市場估算、同類專家常用標籤）、run-split 發包、媒合結果列表、DELETE 單筆 match；單位僅供對照不參與篩選；標籤以 `project_items.requirements.tags` 為準。
- 結論：發包與媒合主流程已打通，下一步為 E2E 測試、媒合結果呈現優化、通知與金流。
...
採事件驅動、可擴充的雲端風格：
  ...
  - **Gemini 3 Flash Preview**: 服務分析、規格拆解、標籤生成
  - **Gemini 3 Pro Image Preview**: 產品圖片生成 ✨ NEW
  - 發案：拆圖 → 3 個工項，每項 3 個規格 Tags。
  - 專家：輸入「品名／價格」→ 5 個隱藏 Tags（SEO＋語義擴展）。
  - **客製產品**：文字描述 → 生成示意圖 → 分析產品特徵 ✨ NEW
  - 發案：拆圖 → 3 個工項，每項 3 個規格 Tags。
  - 專家：輸入「品名／價格」→ 5 個隱藏 Tags（SEO＋語義擴展）。
...
manufacturer'|'admin'), `subscription_tier` ('free'|'expert')
- `experts_profile`
  - `user_id` (FK users), `verified_status` (bool), `skills_tags` (text[]), `rating` (numeric)
- `manufacturers` ✨ NEW
  - `user_id` (FK users), `name` (text), `specialty` (text), `experience` (int), `rating` (numeric), `location` (text), `production_capabilities` (jsonb), `portfolio` (jsonb)
- `listings`（報價單）
...
- `custom_products` ✨ NEW（客製產品發案）
  - `id` (uuid, PK), `owner_id` (FK users), `category` (text), `quantity` (int), `description` (text)
  - `image_urls` (text[]), `is_ai_generated` (bool), `generation_prompt` (text)
  - `ai_analysis` (jsonb) - 包含 productType, materials, techniques, difficulty 等
  - `budget_min` (numeric), `budget_max` (numeric)
  - `matched_manufacturers` (uuid[]), `status` ('draft'|'published'|'matched'|'closed')
...
## 8. 實作路api/generate-product-image` ✨ NEW：輸入文字描述 → 生成產品示意圖（Base64）
- `POST /api/analyze-custom-product` ✨ NEW：輸入產品圖片 → 分析特徵 → 推薦訂製廠商
- `POST /線圖（Roadmap）
