...
| **人像攝影・實驗（FLUX）** | `bfl_flux_model_promo_portrait` + `promo_portrait_experiment_flux_safety_tolerance` | `getPromoPortraitEngineForRenderMode('experiment')` | 與清晰同提示詞、只送 FLUX。臉複製較低。尺度 FLUX.2 官方 **0–5**（預設 2）。模型與氛圍第二段共用 |
