...
### 3.2 媒體四套、文字兩套（2026-09-03 定案）

介面截圖／GIF／影片會因**語系 UI**與**桌面／手機版面**不同，必須分檔。步驟說明文字不必因螢幕各寫一遍。

| | 中文 | 英文 |
|---|------|------|
| **文字**（標題、摘要、區塊正文、圖說） | 一份 | 一份（共用，不依桌面／手機再拆） |
| **圖片／GIF** | 桌面中文 + 行動中文 | 桌面英文 + 行動英文 |
| **YouTube** | 同上四套可填；畫面若相同可只填中英、或只填桌面，缺的 fallback | |

後台一個「圖片／GIF」或「YouTube」區塊用 **語系（中／英）× 畫面（桌面／行動）切換**，一次只編目前環境的那一檔；四套仍寫入 `blocks_json`。圖說仍只有「圖說」「圖說（英文）」。

前台同一時間只顯示 **一張圖或一支影片**：`?lang=` 先選中或英，再用寬度選桌面或行動。缺檔 fallback 順序（例：英文＋手機）→ 行動英 → 桌面英 → 行動中 → 桌面中／舊欄 `url`。舊資料只有單張 `url` 時當成桌面中文。桌面與行動 URL 相同時只輸出一個 `<img>`／一個 iframe。

**已實作（2026-09-03）**：後台 `/admin/operation-guides.html` 以環境切換編輯四套媒體（非 2×2 並排）；`blocks_json` 存 `url_desktop_zh`…／`video_id_desktop_zh`…，舊欄 `url`／`video_id`＝桌面中文。前台 `lib/help-guides.js` 用一組 `<picture>`（或單 `<img>`）與至多雙 iframe（`d-none d-md-block`／`d-md-none`，同時只見一個）。文字仍兩套。翻譯 API 只翻圖說，不翻網址。無需新 SQL。
