...
1. 材料組合 Step2 **現行主線是 Gemini 生圖**；勿再把三色寫成「FLUX prompt 工程」。  
2. **前端 UI 嚴禁出現任何模型名稱**。  
3. **手動輸入色號＋手動儲存是主路徑**；「套用」只填表單。  

**比重定案（2026-08-06）**  
- 雙色：固定 **75/25（預設，維持現況）**＋**50/50**；不做任意％。（曾提 80/20，已改回 75/25）  
- 三色：**自訂％**（合計必須 100）；色塊圖用垂直 `floor`＋最後一段吃餘數畫 1024 PNG——見 PLAN §2.3。

實作前若還要改：三色快速預設數字、印花是否仍一區擇一。
