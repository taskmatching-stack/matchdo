...
有印花圖樣時（圖1＝色卡；圖2／圖3＝主／配區圖樣，有選才佔號；**保留該區材質名**）：

```
依原圖上方主色區（尼龍布）改為圖2印花圖樣，下方配色區改為壓紋皮革材質，解析度1024x1024，不需要文字
```

兩區皆有圖樣：

```
依原圖上方主色區（尼龍布）改為圖2印花圖樣，下方配色區（壓紋皮革）改為圖3印花圖樣，解析度1024x1024，不需要文字
```

有分界處時於句中插入 `，分界處改為{分界處描述}`。
