...
| 參考圖 | Step1 色卡整張（`input_image`）；可選主／配區印花圖樣為 `input_image_2`／`input_image_3` |
...
## Step2 路徑（勿再靠加長 prompt 擋印花滲色）

| 條件 | 管線 |
|------|------|
| **無印花** | 整張色卡一次 img2img（`disable_pup`；已驗證可保住 2/3·1/3） |
| **任一區有印花** | 上／下色帶**各自** FLUX（該區印花只進該區），再硬拼 2/3·1/3。禁止整張附印花參考圖 |

單區實際送出（有印花＋材質）：

```
整張改為圖2印花圖樣的尼龍布材質，解析度1024x1024，不需要文字
```

展示用合成句仍可寫「上方主色區…下方配色區…」。
