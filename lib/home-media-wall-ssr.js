...
const manufacturerAudience = require('./manufacturer-audience');
...
async function fetchHomeMediaWallSsrItems(opts) {
    ...
    if (items.length < limit) {
        try {
            const need = Math.max(8, limit - items.length);
            const visiblePorts = await manufacturerAudience.fetchPortfolioRowsForMediaWallAudience(supabase, {
                limit: need,
                startOffset: 0,
                internalPreview: internalPreview,
                requireDisplayableMedia: true,
                buildRangeQuery: function (from, to) {
                    return supabase
                        .from('manufacturer_portfolio')
                        .select('id, manufacturer_id, title, description, ai_tags, image_url, image_url_before, show_on_media_wall, series_image_valid_until, series_image_urls, created_at')
                        .eq('show_on_media_wall', true)
                        .order('created_at', { ascending: false })
                        .range(from, to);
                }
            });
            visiblePorts.forEach((r) => {
                if (!r || !r.id) return;
                const type = r.image_url_before ? 'comparison' : 'series';
                const img = r.image_url || r.image_url_before;
                if (!img) return;
                const tags = Array.isArray(r.ai_tags) ? r.ai_tags.map((t) => String(t || '').trim()).filter(Boolean) : [];
                items.push({
                    type: type,
...
function applyHomeSsrToHtml(html, opts) {
    ...
    if (gridHtml) {
        const next = out.replace(
            /<div id="media-wall-grid">[\s\S]*?<\/div>/,
            '<div id="media-wall-grid">' + gridHtml + '</div>'
        );
