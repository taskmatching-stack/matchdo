...
async function resolveEmbedPlanTier(supabase, vendorUserId) {
    ...
    if (error && isMissingColumnError(error, 'embed_enabled')) {
        ...
        const planOnly = retry.data && retry.data.subscription_plans;
        if (!planOnly) return null;
        const keyOnly = planOnly.plan_key ? String(planOnly.plan_key).trim() : '';
        if (keyOnly && EMBED_TIER_BY_PLAN_KEY[keyOnly]) return EMBED_TIER_BY_PLAN_KEY[keyOnly];
        const priceOnly = Number(planOnly.price);
        if (EMBED_TIER_BY_PRICE[priceOnly]) return EMBED_TIER_BY_PRICE[priceOnly];
        if (priceOnly >= 1800) return '1800';
        if (priceOnly >= 900) return '900';
        if (priceOnly >= 300) return '300';
        return null;
    }
    if (error && !isMissingColumnError(error, 'embed_enabled')) {
    ...
    if (!plan) return null;
    if (plan.embed_enabled === false) return null;
    const key = plan.plan_key ? String(plan.plan_key).trim() : '';
    if (key && EMBED_TIER_BY_PLAN_KEY[key]) return EMBED_TIER_BY_PLAN_KEY[key];
    const price = Number(plan.price);
    if (EMBED_TIER_BY_PRICE[price]) return EMBED_TIER_BY_PRICE[price];
    if (plan.embed_enabled === true && price >= 300) {
...
async function assertEmbedInstanceQuota(supabase, manufacturerId, vendorUserId) {
    ...
    if (!tier) {
        throw new EmbedSimulatorError('embed_disabled', 403, 'Embed 試做僅限付費會員（300／900／1800 方案）');
    }
    const cfg = getEmbedTierConfig(tier);
...
const EMBED_INSTANCE_SELECT_FULL =
    'id, manufacturer_id, name, embed_key, embed_secret, prototype_asset_id, allowed_origins, rate_limit_per_ip_hour, daily_cap, monthly_cap, is_active, show_on_media_wall';
