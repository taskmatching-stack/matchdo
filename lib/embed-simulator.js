...
const EMBED_TIER_CONFIG = {
    '300': { maxInstances: 3, showPoweredBy: true, wallMode: 'always', sceneSimMode: 'always' },
    '900': { maxInstances: null, showPoweredBy: true, wallMode: 'always', sceneSimMode: 'configurable' },
    '1800': { maxInstances: null, showPoweredBy: false, wallMode: 'configurable', sceneSimMode: 'configurable' }
};
...
/** 後台 profiles.member_level（無 user_subscriptions 時仍視為付費）→ embed 等級 */
function embedTierFromMemberLevel(memberLevel) {
    const level = (memberLevel != null ? String(memberLevel).trim() : '') || '一般';
    if (!level || level === '一般') return null;
    if (level === 'VIP') return '1800';
    if (level === '尊榮') return '900';
    return '300';
}

async function resolveEmbedPlanTier(supabase, vendorUserId) {
    if (!vendorUserId) return null;
    const { data: profile } = await supabase.from('profiles').select('role, member_level').eq('id', vendorUserId).maybeSingle();
    ...
    if (!bestTier && profile) {
        const tierFromLevel = embedTierFromMemberLevel(profile.member_level);
        if (tierFromLevel) bestTier = tierFromLevel;
    }
    if (!bestTier && (rows || []).some(function (row) {
        const plan = normalizeSubscriptionPlan(row.subscription_plans);
...
/** 公開 iframe：實境模擬是否開啟（300 強制開；900/1800 讀實例欄位） */
function resolveEmbedSceneSimFromInstance(instanceRow, tier) {
    const cfg = getEmbedTierConfig(tier);
    if (!cfg) return false;
    if (cfg.sceneSimMode === 'always') return true;
    if (cfg.sceneSimMode === 'configurable') {
        return !!(instanceRow && instanceRow.scene_sim_enabled !== false);
    }
    return false;
}

function resolveEmbedSceneSimVisible(tier, instanceRow) {
    return resolveEmbedSceneSimFromInstance(instanceRow, tier);
}
...
async function assertEmbedInstanceQuota(supabase, manufacturerId, vendorUserId) {
    ...
    return assertEmbedInstanceQuotaWithTier(supabase, manufacturerId, tier);
}

async function assertEmbedInstanceQuotaWithTier(supabase, manufacturerId, tier) {
    if (!tier) {
        throw new EmbedSimulatorError('embed_disabled', 403, '需為付費會員才可取得嵌入碼（後台會員等級進階以上，或有效付費訂閱）');
    }
    const cfg = getEmbedTierConfig(tier);
...
function isEmbedOptionalColumnError(err, colName) {
    if (!err) return false;
...
function isMissingColumnError(err, col) {
    if (isEmbedOptionalColumnError(err, col)) return true;
    return err && err.code === '42703' && String(err.message || '').includes(col);
}

const EMBED_INSTANCE_SELECT_FULL =
    'id, manufacturer_id, name, embed_key, embed_secret, prototype_asset_id, allowed_origins, rate_limit_per_ip_hour, daily_cap, monthly_cap, is_active, show_on_media_wall, show_powered_by, scene_sim_enabled';
...
async function fetchEmbedInstanceByKey(supabase, embedKey) {
    let result = await supabase
        ...
        .maybeSingle();
    if (result.error && isEmbedOptionalColumnError(result.error)) {
        ...
        if (!result.error && result.data) {
            ...
            result.data.scene_sim_enabled = true;
        }
        ...
        if (result.data.scene_sim_enabled === undefined) result.data.scene_sim_enabled = true;
    }
    return result;
}
...
async function resolveEmbedInstance(supabase, embedId, sig) {
    ...
    if (row && row.scene_sim_enabled === undefined) row.scene_sim_enabled = true;
...
async function chargeVendorCredits(supabase, userId, amount, description, source) {
    ...
    await supabase.from('credit_transactions').insert({
        ...
        source: source || 'embed_simulator_generate',
...
function manufacturerProfileUrlFromRow(mfr) {
    if (!mfr || !mfr.id) return null;
    return '/vendor-profile.html?id=' + encodeURIComponent(String(mfr.id));
}
...
function normalizeEmbedUiQuery(opts) {
    opts = opts || {};
    let theme = String(opts.theme || '').trim().toLowerCase();
    let chrome = String(opts.chrome || '').trim().toLowerCase();
    let lang = String(opts.lang || '').trim();
    if (theme !== 'host' && theme !== 'light' && theme !== 'dark') theme = '';
    if (chrome !== 'minimal' && chrome !== 'none' && chrome !== 'full') chrome = '';
    if (lang !== 'en' && lang !== 'zh-TW') lang = '';
    return { theme, chrome, lang };
}

function buildSimulatorPagePath(embedKey, sig, uiOpts) {
    let path = '/embed/simulator.html?embed_id=' + encodeURIComponent(String(embedKey || '')) +
        '&sig=' + encodeURIComponent(String(sig || ''));
    const q = normalizeEmbedUiQuery(uiOpts);
    if (q.theme) path += '&theme=' + encodeURIComponent(q.theme);
    if (q.chrome) path += '&chrome=' + encodeURIComponent(q.chrome);
    if (q.lang) path += '&lang=' + encodeURIComponent(q.lang);
    return path;
}

function applyEmbedUiParamsToUrl(pageUrl, opts) {
    const base = String(pageUrl || '');
    if (!base) return '';
    const q = normalizeEmbedUiQuery(opts);
    try {
        const u = new URL(base, 'https://matchdo.cc');
        if (q.theme) u.searchParams.set('theme', q.theme);
        else u.searchParams.delete('theme');
        if (q.chrome) u.searchParams.set('chrome', q.chrome);
        else u.searchParams.delete('chrome');
        if (q.lang) u.searchParams.set('lang', q.lang);
        else u.searchParams.delete('lang');
        // 相對 path 回傳（無 origin 時）
        if (!/^https?:\/\//i.test(base)) {
            return u.pathname + u.search + (u.hash || '');
        }
        return u.toString();
    } catch (_) {
        let out = base.replace(/([?&])(theme|chrome|lang)=[^&]*/g, '').replace(/[?&]$/, '');
        const parts = [];
        if (q.theme) parts.push('theme=' + encodeURIComponent(q.theme));
        if (q.chrome) parts.push('chrome=' + encodeURIComponent(q.chrome));
        if (q.lang) parts.push('lang=' + encodeURIComponent(q.lang));
        if (!parts.length) return out;
        return out + (out.indexOf('?') >= 0 ? '&' : '?') + parts.join('&');
    }
}

function buildSimulatorPageUrl(origin, embedKey, secret, uiOpts) {
    const sig = computeEmbedSig(embedKey, secret);
    const path = buildSimulatorPagePath(embedKey, sig, uiOpts);
...
function buildSimulatorIframeSnippet(pageUrl, opts) {
    ...
    if (!Number.isFinite(h) || h < 200) h = 720;
    const q = normalizeEmbedUiQuery(opts);
    const titleDefault = q.lang === 'en' ? 'Try this product' : '產品試做';
    const title = String(opts.title || titleDefault).replace(/"/g, '&quot;');
    const src = applyEmbedUiParamsToUrl(pageUrl, opts).replace(/"/g, '&quot;');
    const radius = opts.borderRadius != null ? Number(opts.borderRadius) : 8;
    const r = Number.isFinite(radius) && radius >= 0 ? radius : 8;
    return '<iframe\n  src="' + src + '"\n  width="100%"\n  height="' + h + '"\n  style="border:0;border-radius:' + r + 'px;max-width:100%;"\n  loading="lazy"\n  referrerpolicy="no-referrer-when-downgrade"\n  title="' + title + '"\n></iframe>';
}
...
function mapEmbedInstanceForVendor(row, origin, opts) {
    ...
    const uiOpts = Object.assign({ theme: 'host', chrome: 'minimal', height: 720 }, opts || {});
    const pageUrl = buildSimulatorPageUrl(origin, row.embed_key, row.embed_secret, uiOpts);
    const sig = computeEmbedSig(row.embed_key, row.embed_secret);
    return {
        id: row.id,
        name: row.name || '',
        ...
        scene_sim_enabled: row.scene_sim_enabled !== false,
        page_url: pageUrl,
        page_url_base: buildSimulatorPageUrl(origin, row.embed_key, row.embed_secret, {}),
        iframe_snippet: buildSimulatorIframeSnippet(pageUrl, uiOpts),
        preview_path: buildSimulatorPagePath(row.embed_key, sig, uiOpts),
        embed_ui_defaults: { theme: 'host', chrome: 'minimal', height: 720 }
    };
}
...
module.exports = {
    ...
    resolveEmbedSceneSimFromInstance,
    resolveEmbedSceneSimVisible,
    countActiveEmbedInstances,
    assertEmbedInstanceQuota,
    assertEmbedInstanceQuotaWithTier,
    ...
    manufacturerProfileUrlFromRow,
    sendEmbedError,
    buildSimulatorPagePath,
    buildSimulatorPageUrl,
    buildSimulatorIframeSnippet,
    applyEmbedUiParamsToUrl,
    normalizeEmbedUiQuery,
    randomEmbedKey,
    randomEmbedSecret,
