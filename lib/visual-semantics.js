...
const MATERIAL_PROMPT_VERSION = 'v9-material-cover-image-first';
...
- intent_summary 用一行：繁中說明，後接「 / 」再寫一句英文；**繁中前半寫入 title、英文句寫入 title_en**（靈感牆依語系顯示），須具體描述圖中品項，禁止「產品設計稿」「產品設計圖」「未命名」「訂製產品」「Untitled」「Product design」「Product design draft」「Design draft」等通用詞。
...
const PROMO_CAMERA_SEMANTICS_PROMPT_VERSION = 'v1-natural-bilingual';

const PROMO_CAMERA_SEMANTICS_JSON_EXAMPLE = `{
  "description_zh": "（2～4 句台灣繁體中文，自然口吻描述這張成圖；以圖為準，勿 markdown）",
  "description_en": "(2–4 English sentences, natural tone, image-first, no markdown)",
  "tags": ["繁中檢索詞", "english search term"],
  "intent_summary": "（一行：繁中 / English）",
  "locale": "zh-TW+en"
}`;

/** 商攝導演成圖：自然描述 + 雙語 tags（與情境圖 TAB 分開） */
const PROMO_CAMERA_NATURAL_RULE = `語系與語氣（必守）：
- description_zh 必用台灣繁體中文，嚴禁簡體字。
- description_zh 與 description_en 各 2～4 句，自然口吻，像在跟同行描述這張片子；勿寫操作步驟、勿寫「請先」「Tab」「步驟一」。
- 先寫眼睛看到的畫面（主體、光、氛圍），再帶一句這類片子品質靠什麼（光線、構圖、格局、主題、材質與採光等），以圖為準。
- tags 陣列 10～20 個，須同時含繁體中文與英文（約各半），供搜尋與分類；與描述分工，勿把描述整段拆成 tags。
- intent_summary：一行濃縮，繁中後接「 / 」再寫英文。
- 附帶生成資訊僅供對照，不可替代以圖為準。`;
...
const SEMANTICS_PROMPT_KEYS = [
    ...
    'promo_camera_product_semantics_prompt',
    'promo_camera_space_layout_semantics_prompt',
    'promo_camera_space_eye_semantics_prompt',
    'promo_camera_portrait_semantics_prompt'
];
...
【材料封面圖 — 先判斷版型，再以像素為準（勿預設多格）】
材料「封面」可能是下列任一型態，**必須依影像判斷**，不可一律當成多色組合：
A. **多格拼合封面**：同一張圖內可見 2～9 格，每格為單色樣張（例：2×2、1×3）。請逐格讀取各格可見的材質、紋理、色相、光澤後**合併**寫入 JSON：colors 列出各格主要色；若各格為**同一材質**不同色，materials／patterns／craftsmanship 寫共用表面特徵（例：荔枝紋皮革、pebbled leather），勿把「多色／色卡／組合材料」當成材質名稱。
B. **單張滿版封面**：整張圖為單一材質表面（滿版布紋、石材、木紋、金屬板等）。依下方一般材料規則分析，**不要**寫成多格色卡或多色組合。
- 格子分隔線、外框、留白僅為排版，**不是**材質紋理。
- intent_summary／product_description 的主體須是**實際材質**（例：流紋大理石、丹寧斜紋、拉絲不鏽鋼）；僅在 A 型且確實多色時可簡述「含 N 色樣／N-color swatch set」，禁止空泛的「彩色組合材料」「multi-color composite material」當主標或 category_guess。
...
- 整份 JSON 須自洽：單張滿版時各欄描述同一表面；多格封面且各格同材質時，materials／patterns／craftsmanship 描述共用表面、colors 涵蓋各格色相；各格材質明顯不同時，materials／intent_summary 須具體點出系列內容（例：皮革＋麂皮色卡系列），不可並列互斥形容或只寫「組合材料」。
...
${PROMO_SCENE_SEMANTICS_JSON_EXAMPLE}`,
    ...
    promo_camera_product_semantics_prompt: `你是 MatchDO 商攝導演「產品攝影」成圖的視覺分析專家。請讀圖後產出自然描述與雙語 tags，幫助想拍產品廣告的人理解這類成圖的品質與做法。

任務：以圖為準，描述這張產品商攝成圖（棚拍或情境置入皆可）。讀者應能從描述理解畫面長什麼樣、光與背景如何、這類片子靠什麼撐起質感。

${PROMO_CAMERA_NATURAL_RULE}

只輸出一段 JSON，不要 markdown。
JSON 格式：
${PROMO_CAMERA_SEMANTICS_JSON_EXAMPLE}`,

    promo_camera_space_layout_semantics_prompt: `你是 MatchDO 商攝導演「空間攝影」的 ISO／俯視空間地圖（layout_plan）視覺分析專家。這不是走進去拍的室內實景照，而是空間配置示意圖。

任務：以圖為準，自然描述這張空間地圖（視角、格局分區、風格材質、整體調性）。讀者應能理解這類空間圖的品質來自格局是否清楚、風格是否一致。
...
${PROMO_CAMERA_SEMANTICS_JSON_EXAMPLE}`,
    ...
    promo_camera_space_eye_semantics_prompt: `你是 MatchDO 商攝導演「空間攝影」的人眼平視室內攝影（eye_level）視覺分析專家。這是平視實景感成片，不是 ISO 空間地圖。

任務：以圖為準，自然描述機位所見（取景範圍、光線、材質、空間調性）。讀者應能理解這類平視空間照的品質來自視線、窗光與材質層次。
...
${PROMO_CAMERA_SEMANTICS_JSON_EXAMPLE}`,
    ...
    promo_camera_portrait_semantics_prompt: `你是 MatchDO 商攝導演「人像攝影」成圖的視覺分析專家。

任務：以圖為準，自然描述景別、造型、光與背景、整體商業人像調性。讀者應能理解這類人像照的品質來自主題氛圍、光型與造型，而非複雜場景。
...
function resolvePromoCameraSemanticsPromptKey(shootMode, spaceOutputType) {
    const shoot = String(shootMode || 'product').trim().toLowerCase();
    if (shoot === 'space') {
        const out = String(spaceOutputType || 'layout_plan').trim().toLowerCase();
        return out === 'eye_level'
            ? 'promo_camera_space_eye_semantics_prompt'
            : 'promo_camera_space_layout_semantics_prompt';
    }
    if (shoot === 'portrait') return 'promo_camera_portrait_semantics_prompt';
    return 'promo_camera_product_semantics_prompt';
}

function resolvePromoCameraTaggingPromptText(dbPrompt, promptKey) {
    const v = (dbPrompt || '').trim();
    const def = DEFAULT_PROMPTS[promptKey] || DEFAULT_PROMPTS.promo_camera_product_semantics_prompt || '';
    if (!v) return def;
    if (v.length < 200 || !/description_zh/.test(v)) return def;
    return v;
}

/** 商攝成圖 description 欄：繁中段落 + 空行 + 英文段落 */
function buildPromoCameraDescriptionFromSemantics(semantics) {
    if (!semantics || typeof semantics !== 'object') return null;
    const zh = (semantics.description_zh || semantics.product_description_zh || '').trim();
    const en = (semantics.description_en || semantics.product_description_en || '').trim();
    if (zh && en) return zh + '\n\n' + en;
    return buildVendorAssetDescriptionFromSemantics(semantics);
}
...
function parseSemanticsJson(responseText) {
    ...
    try {
        ...
        const descriptionZh = (obj.description_zh != null ? String(obj.description_zh).trim() : '')
            || (obj.product_description_zh != null ? String(obj.product_description_zh).trim() : '');
        const descriptionEn = (obj.description_en != null ? String(obj.description_en).trim() : '')
            || (obj.product_description_en != null ? String(obj.product_description_en).trim() : '');
        const hasDesc = descriptionZh || descriptionEn;
        ...
        const semantics = {
            ...
            product_description_zh: descriptionZh
                || (obj.product_description_zh != null ? String(obj.product_description_zh).trim() : null),
            product_description_en: descriptionEn
                || (obj.product_description_en != null ? String(obj.product_description_en).trim() : null),
...
async function analyzeImageSemanticsMulti(deps, imageEntries, context = {}) {
    ...
    if (isMaterial && context.material_cover_composite) {
        ctxLines.push('此圖為材料庫「封面」。先依像素判斷：若可見多格樣張則逐格解讀後合併標籤（寫實際材質名，勿空泛「彩色組合」）；若為單張滿版材質則依單面分析，勿假設多格。');
    }
...
async function analyzePromoCameraImageSemantics(deps, imagePart, context = {}) {
    const { supabase, genAI, runInGeminiQueue, getTaggingModelName: getModel } = deps;
    if (!process.env.GEMINI_API_KEY) throw new Error('未設定 GEMINI_API_KEY');
    const model = await (getModel ? getModel() : getTaggingModelName(supabase));
    const promptKey = resolvePromoCameraSemanticsPromptKey(context.shoot_mode, context.space_output_type);
    const basePrompt = resolvePromoCameraTaggingPromptText(await getConfigPrompt(supabase, promptKey), promptKey);
    let promptText = basePrompt;
    ...
    const parsed = parseSemanticsJson(text);
    if (!parsed) throw new Error('無法解析商攝成圖語意');
    return {
        ...parsed,
        model: usedModel,
        prompt_version: PROMO_CAMERA_SEMANTICS_PROMPT_VERSION,
...
const GENERIC_PRODUCT_TITLES = new Set(['產品設計稿', '產品設計圖', '未命名', '推廣圖', '情境圖', 'Untitled', 'Product design', 'Product design draft', 'Design draft', 'Design drafts', 'Custom product', 'Promo image']);
...
module.exports = {
    ...
    analyzePromoCameraImageSemantics,
    resolvePromoCameraSemanticsPromptKey,
    buildPromoCameraDescriptionFromSemantics,
    bufferToImagePart,
    fetchUrlToImagePart,
    mergeTags,
