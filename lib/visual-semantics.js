...
- intent_summary、product_description：只寫材質種類、紋理、配色、光澤／工藝，勿寫球體、色卡架、渲染、室內／建築場景。`;
...
const MATERIAL_SEMANTICS_JSON_EXAMPLE = `{
  ...
  "intent_summary": "黑白大理石拋光飾材 / Polished black-and-white marble surface",
...
async function analyzeImageSemantics(deps, imagePart, context = {}) {
    ...
    const basePrompt = await getConfigPrompt(supabase, promptKey);
    const ctxLines = [];
 ...
 */
/** 材料標題：僅用材質種類名（materials／category_guess），不用紋理或場景詞 */
function pickZhEnPairForMaterialTitle(semantics, hints) {
    const pair = pickZhEnPair([semantics.materials], {
        materialCatalogHint: hints && hints.materialCatalogHint,
        categoryGuess: semantics.category_guess
    });
    if (pair.zh && pair.en) return pair;
    if (pair.zh && !pair.en) {
        const cg = splitBilingualSlash(semantics.category_guess || '');
        if (cg.en) return { zh: pair.zh, en: normalizeEnTitlePart(cg.en) };
    }
    if (!pair.zh && pair.en) {
        const cg = splitBilingualSlash(semantics.category_guess || '');
        if (cg.zh) return { zh: cg.zh, en: pair.en };
    }
