...
function buildMaterialFluxFidelityLine(semanticsJson) {
    ...
    pick('surface materials', s.materials, 6);
    pick('colors', s.colors, 6);
    pick('texture and pattern', s.patterns, 6);
    pick('surface finish', s.craftsmanship, 4);
    if (s.intent_summary) {
        chunks.push(`visual summary: ${String(s.intent_summary).trim()}`);
    }
    if (!chunks.length && Array.isArray(s.tags) && s.tags.length) {
        const tags = filterMaterialTagList(s.tags, 12);
        if (tags.length) chunks.push(`visual character: ${tags.join(', ')}`);
    }
    if (!chunks.length) return '';
    return `Material characteristics from Gemini vision analysis of the reference image (preserve exactly in the output, do not invent or substitute): ${chunks.join('; ')}.`;
}
