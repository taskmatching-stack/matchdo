...
const MATERIAL_PROMPT_VERSION = 'v5-material-single-pattern';
...
2. 紋理 patterns（如流紋、veined、編織紋）— **見下方紋理規則**
3. 配色 colors（如米白、off-white、黑白灰）
4. 光澤／表面工藝 craftsmanship（如拋光、polished、霧面、matte、拉丝、brushed）
- tags 以以上四類為主；每類至少 1 組中英詞。

【必守：紋理 patterns 只能一種主導紋理族】
- 以**相片像素**上實際可見的紋理為準；禁止用標題或檔名猜測。
- 荔枝紋／pebbled、細膩粒面／fine grain、粗粒／coarse grain、平紋／smooth、十字紋／Saffiano、編織／woven 等是**不同族**；只選**一種**最符合的主導族，不可並列兩種以上不同族。
- patterns 陣列最多 2 組中英詞，且必須描述**同一種**紋理（例：["荔枝紋","pebbled grain"]），禁止同時寫荔枝紋與細膩粒面。
- intent_summary 必須與 patterns 同一紋理族，勿合併矛盾形容。
...
/** 紋理族：用於剔除 Gemini 誤併的矛盾紋理（例：荔枝紋 + 細膩粒面） */
const MATERIAL_PATTERN_FAMILY_RULES = [
    { key: 'pebbled', re: /荔枝|pebble|pebbled|litchi|羊京/i },
    { key: 'fine_grain', re: /細膩粒|細粒面|fine\s*grain|micro\s*grain|微粒|小粒/i },
    { key: 'coarse_grain', re: /粗粒|coarse\s*grain|大粒/i },
    { key: 'smooth', re: /平紋|素面|光面|smooth|plain\s*grain/i },
    { key: 'saffiano', re: /十字紋|saffiano|cross[\s-]?grain/i },
    { key: 'woven', re: /編織|woven|weave|織紋/i },
    { key: 'veined', re: /流紋|veined|雲紋|marble\s*grain/i },
    { key: 'wood_grain', re: /木紋|wood\s*grain/i }
];

function materialPatternTagFamily(tag) {
    const t = String(tag || '').trim();
    if (!t) return null;
    for (const rule of MATERIAL_PATTERN_FAMILY_RULES) {
        if (rule.re.test(t)) return rule.key;
    }
    return 'other';
}

/** 只保留第一個主導紋理族（中英各一組），避免 FLUX 收到矛盾紋理指示 */
function collapseConflictingMaterialPatterns(patterns) {
    const list = filterMaterialTagList(normTagArray(patterns, 12), 12);
    if (!list.length) return [];
    let primaryFamily = null;
    const kept = [];
    for (const tag of list) {
        const fam = materialPatternTagFamily(tag);
        if (fam === 'other') {
            if (primaryFamily === null && kept.length < 2) kept.push(tag);
            else if (primaryFamily !== null && kept.length < 4) kept.push(tag);
            continue;
        }
        if (primaryFamily === null) primaryFamily = fam;
        if (fam !== primaryFamily) continue;
        if (kept.length < 4) kept.push(tag);
    }
    return kept.slice(0, 4);
}

/** 材料 FLUX img2img：Gemini 讀原圖後的材質特徵 → 告訴 FLUX 必須保留什麼 */
function buildMaterialFluxFidelityLine(semanticsJson) {
    if (!semanticsJson || typeof semanticsJson !== 'object') return '';
    const s = sanitizeMaterialSemantics(semanticsJson);
    const chunks = [];
    if (s.category_guess) {
        chunks.push(`material type: ${String(s.category_guess).trim()}`);
    }
    const pick = (label, arr, max) => {
        const list = filterMaterialTagList(normTagArray(arr, max), max);
        if (list.length) chunks.push(`${label}: ${list.join(', ')}`);
    };
    pick('colors', s.colors, 4);
    const patterns = collapseConflictingMaterialPatterns(s.patterns);
    if (patterns.length) {
        chunks.push(`dominant texture only (one grain family, match reference pixels): ${patterns.join(', ')}`);
    }
    pick('surface finish', s.craftsmanship, 3);
    if (!chunks.length && Array.isArray(s.tags) && s.tags.length) {
        const tags = filterMaterialTagList(s.tags, 6);
        if (tags.length) chunks.push(`visual character: ${tags.join(', ')}`);
    }
    if (!chunks.length) return '';
    return [
        'Preserve exactly from the reference photograph (enhance only, do not re-synthesize texture)',
        chunks.join('; '),
        'Keep the same grain family, bump scale, and pattern period as the input—never convert pebbled to smooth, fine to coarse, or invent grains not in the photo'
    ].join('. ') + '.';
}
...
function sanitizeMaterialSemantics(semantics) {
    ...
    if (Array.isArray(out.patterns)) {
        out.patterns = collapseConflictingMaterialPatterns(out.patterns);
    }
...
module.exports = {
    ...
    collapseConflictingMaterialPatterns,
