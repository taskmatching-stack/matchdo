...
function buildOfficialTemplatesHtml(opts) {
    ...
    return '<!DOCTYPE html>\n' +
        ...
        '<link href="/css/morandi-global.css?v=7" rel="stylesheet">\n' +
        '<link href="/css/design-workspace-tabs.css?v=1" rel="stylesheet">\n' +
        ...
        '<p class="ot-lead">平台共用官方版型庫。「用此款進行設計／加入參考圖」帶入設計稿；有關聯材料／配件的原型可「看可搭配」（產品樹）。</p>\n' +
        '<div class="ot-filters">' + cards.categoryFilterLinks('/official-templates/', categories, categoryKey) + '</div>\n' +
        '<p class="ot-meta">共 ' + cards.escapeHtmlText(String(total)) + ' 項' +
        (categoryKey ? '（已篩選分類）' : '') + '</p>\n' +
