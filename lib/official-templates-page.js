...
function designPath(item) {
    ...
    let url = '/custom-product.html?tab=product-design&prototype_asset_id=' + encodeURIComponent(item.id) + '&official=1';
...
function buildOfficialTemplatesHtml(opts) {
    ...
    const title = catObj && catObj.name
        ...
        : '官方版型 - MATCHDO 合做';
    const desc = '瀏覽 MATCHDO 官方版型庫：數位原型、材料與配件。點進獨立作品頁，或以「用此款進行設計」帶入設計稿。';
    ...
    const cards = items.map(function (item) {
        ...
        const insp = inspirationPath(item);
        ...
        const imgHtml = img
            ? '<a href="' + escapeHtmlAttr(insp) + '" class="ot-card-img-link"><img src="' + escapeHtmlAttr(img) + '" alt="' + escapeHtmlAttr(item.title || '') + '" loading="lazy" width="400" height="400"></a>'
        ...
        return (
            ...
            '<h2 class="ot-card-title"><a href="' + escapeHtmlAttr(insp) + '">' + t + '</a></h2>' +
            '<div class="ot-card-actions">' +
            '<a class="btn btn-sm btn-outline-secondary" href="' + escapeHtmlAttr(insp) + '">作品頁</a>' +
    ...
    return '<!DOCTYPE html>\n' +
        ...
        '<p class="ot-lead">平台共用官方版型庫。點「作品頁」看獨立說明；「用此款進行設計／加入參考圖」進入設計稿工具。</p>\n' +
