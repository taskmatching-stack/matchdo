...
function buildOfficialTemplatesHtml(opts) {
    ...
    const title = catObj && catObj.name
        ...
        : '官方版型 - MATCHDO 合做';
    const desc = '瀏覽 MATCHDO 官方版型庫：數位原型、材料與配件。「用此款進行設計／加入參考圖」帶入設計稿；有關聯時可「看可搭配」。';
    ...
    const cards = items.map(function (item) {
        ...
        const hasLinks = isProto && linkCount > 0;
        // 對齊設計頁舊卡：僅原型且 hasLinks 才「看可搭配」；勿加「作品頁」
        const guideBtn = hasLinks
            ? ('<a class="btn btn-sm btn-outline-secondary" href="' + escapeHtmlAttr(matchGuidePath(item)) + '">看可搭配</a>')
            : '';
        const imgHtml = img
            ? ('<div class="ot-card-img-link"><img src="' + escapeHtmlAttr(img) + '" alt="' + escapeHtmlAttr(item.title || '') + '" loading="lazy" width="400" height="400"></div>')
    ...
    return '<!DOCTYPE html>\n' +
        ...
        '<p class="ot-lead">平台共用官方版型庫。「用此款進行設計／加入參考圖」帶入設計稿；有關聯材料／配件的原型可「看可搭配」。</p>\n' +
