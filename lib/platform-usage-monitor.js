...
async function fetchVendorBalances(supabase) {
    ...
    if (!mgmtKey) {
        out.grok.balance_error = '未設定 Management API 金鑰（後台審核友善或 Cloud Run XAI_MANAGEMENT_API_KEY）。生圖用推論金鑰查不到餘額。';
    } else if (!teamId) {
...
async function buildPlatformUsageReport(opts) {
    ...
    const setupHints = [
        ...
        'BFL／Stability 餘額來自各平台官方 API；Replicate 無餘額 API。Grok 生圖金鑰只能查連線；預付美元需 Management API 金鑰（後台審核友善或 Cloud Run XAI_MANAGEMENT_API_KEY）。用量見本站扣點／generation_meta。'
    ].filter(Boolean);
