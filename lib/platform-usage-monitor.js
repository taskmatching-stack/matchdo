...
async function fetchVendorBalances(supabase) {
    const out = {
        ...
        grok: {
            ...
            note: '推論金鑰可查連線；預付美元需 Cloud Run XAI_MANAGEMENT_API_KEY'
        }
    };
    ...
    const mgmtKey = String(process.env.XAI_MANAGEMENT_API_KEY || process.env.XAI_MANAGEMENT_KEY || '').trim();
...
async function buildPlatformUsageReport(opts) {
    ...
    const setupHints = [
        ...
        'BFL／Stability 餘額來自各平台官方 API；Replicate／Grok 推論金鑰僅能查連線。Grok 預付美元需另設 XAI_MANAGEMENT_API_KEY（可選）。用量見本站扣點／generation_meta。'
    ].filter(Boolean);
    ...
    return {
        ...
        external_apis: {
            ...
            note: '本月次數／點數來自 credit_transactions；BFL／Stability 餘額為官方 API 即時查詢。Grok 推論金鑰查連線，預付美元需 Management API。Gemini 生圖與標籤不在此頁統計；商攝實驗 Grok 次數見 generation_meta proxy。'
        }
    };
}
