...
async function collectFluxCountsViaSupabase(supabase, since) {
    if (!supabase) {
        return { flux_design_generations: null, flux_promo_generations: null, promo_total_generations: null, error: 'no_supabase_client' };
    }
    try {
        ...
        if (designRes.error) {
            return { flux_design_generations: null, flux_promo_generations: null, promo_total_generations: null, error: designRes.error.message || String(designRes.error) };
        }
        if (promoRes.error) {
            return { flux_design_generations: null, flux_promo_generations: null, promo_total_generations: null, error: promoRes.error.message || String(promoRes.error) };
        }
        let fluxDesign = 0;
        ...
        (promoRes.data || []).forEach(function (row) {
            if (promoRowIsFlux(row.generation_meta_json)) fluxPromo += 1;
        });
        return {
        ...
        return { flux_design_generations: null, flux_promo_generations: null, promo_total_generations: null, error: e.message || String(e) };
    }
}
...
async function collectExternalApiUsageMonth(supabase, dbUrl, monthStart) {
    ...
    if (!fluxCounts.error) {
        ...
        if (fluxCounts.truncated) out.errors.push('FLUX 紀錄統計：已達掃描上限，數字可能偏低');
    } else if (dbUrl) {
        const fluxPg = await collectFluxCountsViaPg(dbUrl, since);
        if (fluxPg.error && fluxPg.error !== 'no_db_url') out.errors.push('FLUX 紀錄統計：' + fluxPg.error);
...
async function fetchVendorBalances() {
    const out = {
...
async function buildPlatformUsageReport(opts) {
    ...
    let apiCounts = null;
    if (projectRef && mgmtToken) {
        apiCounts = await fetchSupabaseApiCounts(projectRef, mgmtToken, '7d');
        if (apiCounts && apiCounts.error) warnings.push('Management API（7d 請求）：' + apiCounts.error);
    }
    ...
    const setupHints = [
        ...
        'BFL／Stability 餘額來自各平台官方 API；Replicate 僅能查帳號連線，用量見本站扣點統計。'
    ].filter(Boolean);
    ...
    return {
        ...
        external_apis: {
            ...
            note: '本月次數／點數來自 credit_transactions；BFL／Stability 餘額為官方 API 即時查詢。Gemini 生圖與標籤不在此頁統計。'
        }
    };
}
