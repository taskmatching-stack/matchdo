...
async function collectSupabaseStorageObjectsBytes(dbUrl) {
    if (!dbUrl) return { bytes: null, error: null };
    ...
    if (!wrapped.ok) return { bytes: null, error: wrapped.error };
    return { bytes: wrapped.result, error: null };
}
...
async function collectLegacySupabaseUrlRows(dbUrl) {
    if (!dbUrl) return { total: null, items: [], error: null };
    const wrapped = await withPlatformPgPool(dbUrl, async function (pool) {
        const items = [];
        let total = 0;
    ...
    if (!wrapped.ok) return { total: null, items: [], error: wrapped.error };
...
async function collectCreditUsageViaSupabase(supabase, since) {
    if (!supabase) return { by_source: [], error: 'no_supabase_client' };
    try {
        const { data: rows, error } = await supabase
            ...
            .limit(100000);
        if (error) return { by_source: [], error: error.message || String(error) };
        return { by_source: aggregateCreditRowsBySource(rows || []), error: null };
    } catch (e) {
        return { by_source: [], error: e.message || String(e) };
    }
}

async function collectFluxCountsViaPg(dbUrl, since) {
    if (!dbUrl) return { flux_design_generations: null, flux_promo_generations: null, promo_total_generations: null, error: null };
    const wrapped = await withPlatformPgPool(dbUrl, async function (pool) {
        const [designRow, promoFluxRow, promoTotalRow] = await Promise.all([
            pool.query(`
...
async function collectExternalApiUsageMonth(supabase, dbUrl, monthStart) {
    ...
    const fluxCounts = await collectFluxCountsViaPg(dbUrl, since);
    if (fluxCounts.error && fluxCounts.error !== 'no_db_url') {
        out.errors.push('FLUX 紀錄統計：' + fluxCounts.error);
    }
...
async function buildPlatformUsageReport(opts) {
    ...
    const dbSizeRes = await collectDatabaseSizeBytes(dbUrl);
    const storageRes = await collectSupabaseStorageObjectsBytes(dbUrl);
    const legacyUrls = await collectLegacySupabaseUrlRows(dbUrl);
    const external = await collectExternalApiUsageMonth(supabase, dbUrl, monthStart);
    const snapshots = await loadSnapshots(supabase);

    if (dbSizeRes.error) warnings.push('Database 自動查詢：' + dbSizeRes.error);
    if (storageRes.error) warnings.push('Storage 物件自動查詢：' + storageRes.error);
    ...
    let mgmtDb = null;
    if (dbSizeRes.bytes == null && projectRef && mgmtToken) {
        mgmtDb = await fetchManagementDatabaseQuery(
            projectRef,
            mgmtToken,
            'SELECT pg_database_size(current_database())::bigint AS bytes'
        );
        if (mgmtDb && mgmtDb.error) warnings.push('Management API（Database）：' + mgmtDb.error);
    }
    const databaseBytes = dbSizeRes.bytes != null ? dbSizeRes.bytes
    ...
    if (projectRef && mgmtToken) {
        ...
        if (apiCounts && apiCounts.error) warnings.push('Management API（24h 請求）：' + apiCounts.error);
    }

    const latestManual = snapshots.length ? snapshots[0] : null;
    const auto = {
        database_bytes: databaseBytes,
        storage_objects_bytes: storageRes.bytes,
        ...
        data_sources: {
            database: dbSizeRes.bytes != null ? 'supabase_db_url' : (databaseBytes != null ? 'management_api' : (latestManual && latestManual.db_size_mb != null ? 'manual_snapshot' : 'unavailable')),
            storage_objects: storageRes.bytes != null ? 'supabase_db_url' : (latestManual && latestManual.storage_gb != null ? 'manual_snapshot' : 'unavailable'),
            egress: latestManual && latestManual.egress_gb != null ? 'manual_snapshot' : 'unavailable'
        }
    };

    const setupHints = [
        !dbUrl ? '設定 SUPABASE_DB_URL 可自動查 DB／Storage（Session pooler 6543）；若 SSL 失敗仍可用手動快照與 REST 扣點統計。' : null,
        dbUrl && (dbSizeRes.error || storageRes.error) ? 'Postgres 直連失敗時，請在下方填寫 Usage 快照；外部 API 仍由 credit_transactions 統計。' : null,
        !mgmtToken ? '選填 SUPABASE_ACCESS_TOKEN 可顯示近 24h API 請求量。' : null,
        'Egress 請在 Supabase Usage 頁查看後，於下方表單填寫並按「儲存快照」。'
    ].filter(Boolean);
