...
/** Quill 編輯器：把表格包成不可編輯 embed，避免 Quill 剝掉 */
...
/** 切成「一般 HTML」與「表格」片段，供 Quill 分段插入（表格走 embed，不走 dangerouslyPasteHTML） */
function splitHelpHtmlByTables(html) {
    ...
    while ((m = re.exec(s)) !== null) {
        if (m.index > last) {
            parts.push({ type: 'html', content: s.slice(last, m.index) });
        }
        parts.push({ type: 'table', content: m[0] });
        last = m.index + m[0].length;
    }
    if (last < s.length) {
        parts.push({ type: 'html', content: s.slice(last) });
    }
    if (!parts.length && s.trim()) {
        parts.push({ type: 'html', content: s });
    }
    return parts;
}

/** 後台 Quill：清理後包表格 embed */
...
/** 從 Quill root.innerHTML 還原成可儲存／前台用的 HTML */
