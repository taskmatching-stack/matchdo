...
function normalizeReferenceEntry(s, opts) {
    ...
    const entry = {
        ...
        image_url: (function () {
            const raw = String(s && (s.image_url || s.url) || '').trim();
    ...
    const resume = {
        ...
        generation_context: (function () {
            const ctx = {
                ...
                generation_meta: parseResumeJsonObject(row.generation_meta_json)
            };
...
function isUsableResumeImageUrl(url) {
    const s = String(url || '').trim();
    if (!s || /^data:/i.test(s)) return false;
    return /^https?:\/\//i.test(s) || s.startsWith('/');
}

function parseResumeJsonObject(raw) {
    if (!raw) return {};
...
function promoResumeRefTitle(sourceType, role) {
    const t = String(sourceType || '').trim();
    const roleKey = String(role || '').trim();
    if (roleKey === 'floor_plan') return '平面配置';
    if (roleKey === 'scene') return '場景參考';
    if (roleKey === 'style') return '風格參考';
    if (roleKey === 'staging_product') return '陳列產品';
    if (t === 'custom_product' || t === 'digital_asset') return '來源設計稿';
    if (t === 'vendor_asset') return '來源素材';
    return '產品參考圖';
}

async function lookupPromoSourceAssetForResume(supabase, row) {
    const sourceId = String((row && (row.source_id || row.parent_record_id)) || '').trim();
    if (!sourceId) return null;
    try {
        const { data } = await supabase.from('custom_products')
            .select('id, title, ai_generated_image_url, reference_image_url')
            .eq('id', sourceId)
            .maybeSingle();
        const url = data && (data.ai_generated_image_url || data.reference_image_url);
        if (data && isUsableResumeImageUrl(url)) {
            return {
                type: 'custom_product',
                id: data.id,
                source_id: data.id,
                image_url: url,
                url: url,
                title: data.title || '來源設計稿'
            };
        }
    } catch (_) { /* ignore */ }
    try {
        const { data } = await supabase.from('product_promo_generations')
            .select('id, result_image_url, source_image_url')
            .eq('id', sourceId)
            .maybeSingle();
        const url = data && (isUsableResumeImageUrl(data.source_image_url) ? data.source_image_url : data.result_image_url);
        if (data && isUsableResumeImageUrl(url)) {
            return {
                type: 'digital_asset',
                id: data.id,
    ...
    try {
        const { data } = await supabase.from('vendor_assets')
            .select('id, title, image_url')
            .eq('id', sourceId)
            .maybeSingle();
        if (data && isUsableResumeImageUrl(data.image_url)) {
            return {
                type: 'vendor_asset',
                id: data.id,
                source_id: data.id,
...
async function collectPromoSceneResumeRefs(supabase, row, opts) {
    const refs = [];
    const seen = {};
    function pushRaw(raw) {
        if (!raw) return;
        const url = String(raw.image_url || raw.url || '').trim();
        if (!isUsableResumeImageUrl(url) || seen[url]) return;
        seen[url] = true;
        refs.push(normalizeReferenceEntry(Object.assign({}, raw, { image_url: url, url: url }), opts));
    }
    pushRaw({
        type: (row && row.source_type) || 'upload',
        id: row && row.source_id,
        source_id: row && row.source_id,
        image_url: row && row.source_image_url,
        url: row && row.source_image_url,
        title: promoResumeRefTitle(row && row.source_type)
    });
    const meta = parseResumeJsonObject(row && row.generation_meta_json);
    (Array.isArray(meta.reference_images) ? meta.reference_images : []).forEach(function (r) {
        ...
        pushRaw({
            ...
            id: row && row.source_id,
            source_id: row && row.source_id,
            image_url: r.url || r.image_url,
            url: r.url || r.image_url,
            title: promoResumeRefTitle(row && row.source_type, r.role)
        });
    });
    [
        [meta.floor_plan_source_url || meta.layout_reference_url, 'floor_plan'],
        [meta.scene_image_url, 'scene'],
        [meta.style_image_url, 'style'],
        [meta.staging_product_url, 'staging_product']
    ].forEach(function (pair) {
        pushRaw({
            type: 'upload',
            image_url: pair[0],
            url: pair[0],
            title: promoResumeRefTitle('upload', pair[1])
        });
    });
    if (!refs.length) {
        const looked = await lookupPromoSourceAssetForResume(supabase, row);
        if (looked) pushRaw(looked);
    }
    return refs;
}
...
async function buildPromoSceneResume(supabase, id, opts) {
    ...
    const refs = await collectPromoSceneResumeRefs(supabase, row, opts);
...
async function buildProvenanceResume(supabase, opts) {
    ...
    let result;
    if (kind === 'user_design') result = await buildUserDesignResume(supabase, id, opts);
    else if (kind === 'promo_scene') result = await buildPromoSceneResume(supabase, id, opts);
    else if (kind === 'material_combo') result = await buildMaterialComboResume(supabase, id, opts);
    else if (kind === 'print') result = await buildPrintResume(supabase, id, opts);
    else if (kind === 'embed_visitor') result = await buildEmbedVisitorResume(supabase, id, opts);
    else return { error: 'not_implemented', message: '此類型履歷尚未支援' };
    return stripEncapsulatedPromptsForOwner(result, opts && opts.audience ? opts.audience : 'owner');
}

const SHOOT_MODE_ZH = { portrait: '人像攝影', space: '空間攝影', product: '產品攝影' };
const RENDER_MODE_ZH = { clear: '清晰', mood: '氛圍', hybrid: '混合', experiment: '寬鬆尺度' };
const MOOD_STAGE_ZH = { draft: '草稿繪製', look: '氛圍圖' };
const SPACE_OUTPUT_ZH = { layout_plan: '空間地圖', eye_level: '平視攝影' };

function resolvePortraitRenderModeFromMeta(meta) {
    const m = parseResumeJsonObject(meta);
    const kind = String(m.mood_pipeline_kind || '').trim();
    if (kind === 'flux_then_lite') return 'hybrid';
    if (kind === 'lite_then_flux') return 'mood';
    const explicit = String(m.portrait_render_mode || m.render_mode || '').trim().toLowerCase();
    if (explicit === 'clear' || explicit === 'mood' || explicit === 'hybrid' || explicit === 'experiment') return explicit;
    if (String(m.image_provider || '').trim().toLowerCase() === 'grok' || String(m.grok_model || '').trim()) return 'experiment';
    return '';
}

function spaceUseOptionLabel(key) {
    const k = String(key || '').trim();
    if (!k) return '';
    try {
        const t = require('./promo-space-gemini').SPACE_USE_TYPES[k];
        if (t && t.label) return t.label;
    } catch (_) {}
    return k;
}

function spaceLayoutOptionLabel(view) {
    const v = String(view || '').trim();
    if (!v) return '';
    try {
        return require('./promo-space-gemini').spaceLayoutViewLabel(v);
    } catch (_) {
        return v === 'top_down' ? '俯視空間地圖' : (v === 'iso_45' || v === 'iso' ? 'ISO 空間地圖' : v);
    }
}

function buildSelectedOptions(resume) {
    const out = [];
    ...
    const meta = parseResumeJsonObject(ctx.generation_meta);
    const scene = ctx.scene || {};
    const img = resume.image || {};
    const tax = resume.taxonomy || {};

    const shoot = String(meta.shoot_mode || '').trim().toLowerCase();
    if (SHOOT_MODE_ZH[shoot]) add('拍攝類型', SHOOT_MODE_ZH[shoot]);

    const render = resolvePortraitRenderModeFromMeta(meta);
    if (RENDER_MODE_ZH[render]) add('生成風格', RENDER_MODE_ZH[render]);

    const stage = String(meta.mood_stage || '').trim().toLowerCase();
    if (MOOD_STAGE_ZH[stage]) add('階段', MOOD_STAGE_ZH[stage]);

    add('主題', scene.theme_key || meta.theme_key);
    add('場景', scene.scene_key || meta.scene_key);

    const spaceOut = String(meta.space_output_type || '').trim().toLowerCase();
    if (SPACE_OUTPUT_ZH[spaceOut]) add('輸出類型', SPACE_OUTPUT_ZH[spaceOut]);
    add('空間用途', spaceUseOptionLabel(meta.space_use_type));
    add('地圖視角', spaceLayoutOptionLabel(meta.space_layout_view));

    add('長寬比', img.aspect_ratio || meta.aspect_ratio);
    if (img.width && img.height) add('輸出尺寸', img.width + '×' + img.height);
    if (img.megapixels != null && img.megapixels !== '') add('解析度', img.megapixels + ' MP');

    add('分類', tax.category_key);
    add('子分類', tax.subcategory_key);
    return out;
}

function stripEncapsulatedPromptsForOwner(result, audience) {
    if (!result || !result.resume) return result;
    result.resume.selected_options = buildSelectedOptions(result.resume);
    if (audience === 'admin') return result;
    const p = result.resume.prompts;
    if (p) {
        result.resume.prompts = {
            user_prompt: p.user_prompt || null,
            seed: p.seed != null ? p.seed : null
        };
    }
    if (result.resume.generation_context) {
        delete result.resume.generation_context.generation_meta;
    }
    return result;
}
...
module.exports = {
    ...
    resolvePortraitRenderModeFromMeta,
    RENDER_MODE_ZH
};
