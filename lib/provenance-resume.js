...
const RENDER_MODE_ZH = { mood: '氛圍', clear: '清晰' };
...
function buildSelectedOptions(resume) {
    ...
    const meta = (ctx.generation_meta && typeof ctx.generation_meta === 'object') ? ctx.generation_meta : {};
    ...
    const render = String(meta.portrait_render_mode || meta.render_mode || '').trim().toLowerCase();
    if (RENDER_MODE_ZH[render]) add('呈現', RENDER_MODE_ZH[render]);
