...
/**
 * 人像攝影 — Nano Banana Pro（與空間同一套 Interactions／2K 起）
 * 參考圖 1＝人像身份；可選場景／道具圖；攝影參數以 cameraBlock 帶入。
 */
function buildPromoPortraitGeminiPrompt(opts) {
    const o = opts && typeof opts === 'object' ? opts : {};
    const w = Math.max(2048, parseInt(o.width, 10) || 2048);
    const h = Math.max(2048, parseInt(o.height, 10) || 2048);
    ...
    parts.push(
        'Commercial portrait photography of the same person as reference image 1. '
        + 'Keep the face, facial structure, age, and identity from image 1 unchanged. '
        + 'Theme and scene control atmosphere, environment, and wardrobe only — never invent a different person.'
    );
    if (hasSceneImage && hasStagingProduct) {
        ...
        parts.push(
            'Reference image 2 is a product or prop to feature. Integrate it naturally while keeping the same face as image 1.'
        );
    }
    if (themeLabel || themePrompt || themeComposition) {
        parts.push(
            ['Shoot atmosphere (not a new character)', themeLabel, themePrompt, themeComposition].filter(Boolean).join(': ')
        );
    }
    if (hasSceneImage) {
        ...
        parts.push(
            ['Scene environment', sceneLabel, scenePrompt, sceneComposition].filter(Boolean).join(': ')
        );
    }
    if (userPrompt) {
        parts.push('Edit instruction (change scene/action/wardrobe as asked; do not change facial identity): ' + userPrompt);
    }
    if (shotBrief) parts.push('This shot variation (framing/pose only): ' + shotBrief);
    if (cameraBlock) parts.push('Camera and exposure: ' + cameraBlock);
    parts.push('Pose may adapt to the scene. No text, labels, logos, or watermarks in the image.');
    parts.push('Output resolution ' + w + 'x' + h + ' (minimum 2K long edge).');
