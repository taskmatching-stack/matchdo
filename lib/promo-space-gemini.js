...
function buildPromoPortraitGeminiPrompt(opts) {
    ...
    const refFraming = promoPortraitStyling.normalizePortraitStylingMode(
        o.stylingMode || o.portrait_styling_mode
    ) === 'reference'
        ? promoPortraitStyling.buildPortraitReferenceCommercialFramingGeminiLine()
        : '';
    if (origUserPrompt) {
        if (refFraming) parts.push(refFraming);
