...
/** 内部 tier（2k/4k）→ 官方 imageSize（2K/4K；K 须大写） */
 ...
 *
 * ***捨棄原圖ISO視角***，***禁止俯視圖***，***移除文字、字母、標註***，解讀這個地圖後幫我重繪呈現***人在此場景內從B看向C的低視角***，視角內的格局和結構不能和原圖衝突，室內設計用的商業攝影圖
 */
const EYE_LEVEL_PROMPT_TEMPLATE =
    '***捨棄原圖ISO視角***，***禁止俯視圖***，***移除文字、字母、標註***，解讀這個地圖後幫我重繪呈現***人在此場景內從{from}看向{to}的低視角***，視角內的格局和結構不能和原圖衝突，室內設計用的商業攝影圖';
...
function buildPromoSpaceEyeLevelExplicitGeminiPrompt(opts) {
    ...
    /* 主句必須與此字串一致（*** 範圍不可改） */
    const base =
        '***捨棄原圖ISO視角***，***禁止俯視圖***，***移除文字、字母、標註***，解讀這個地圖後幫我重繪呈現***人在此場景內從'
        + from
        + '看向'
        + to
        + '的低視角***，視角內的格局和結構不能和原圖衝突，室內設計用的商業攝影圖';
