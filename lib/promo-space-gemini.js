...
const SPACE_USE_TYPES = {
    ...
    stage: {
        label: '舞臺／活動空間',
        label_en: 'Stage / event',
        layoutLabel: '舞臺／活動空間',
        layoutLabel_en: 'stage / event venue'
    },
    exhibition: {
        label: '展覽空間',
        label_en: 'Exhibition',
        layoutLabel: '展覽空間',
        layoutLabel_en: 'exhibition space'
    },
    hotel: {
        label: '飯店／民宿空間',
        label_en: 'Hotel / B&B',
...
const SPACE_ZONE_INTENTS = {
    ...
    stage: [
        { key: 'stage', name: '舞臺', name_en: 'Stage', intent_brief: 'performance stage', sort_order: 10 },
        { key: 'seating', name: '觀眾席', name_en: 'Audience seating', intent_brief: 'audience seating', sort_order: 20 },
        { key: 'backstage', name: '後台', name_en: 'Backstage', intent_brief: 'backstage', sort_order: 30 },
        { key: 'lobby', name: '入口大廳', name_en: 'Entry lobby', intent_brief: 'entry lobby', sort_order: 40 },
        { key: 'event_floor', name: '活動區', name_en: 'Event floor', intent_brief: 'event floor', sort_order: 50 }
    ],
    exhibition: [
...
const promoPortraitStyling = require('./promo-portrait-styling');
...
function buildPromoPortraitGeminiPrompt(opts) {
    ...
    const moodDraft = o.moodDraft === true;
    const parts = [];
    const ignorePose = promoPortraitStyling.buildPortraitIgnoreRefPoseEmphasisLine(
        o.themeKey,
        o.stylingMode || o.portrait_styling_mode
    );
    if (ignorePose) parts.push(ignorePose);

    const stylingLead = promoPortraitStyling.buildPortraitStylingGeminiLead(
        o.stylingMode || o.portrait_styling_mode,
        o.themeKey
    );
    parts.push(
        (moodDraft ? 'Commercial portrait. ' : 'Commercial portrait photography. ')
        + stylingLead
    );
    ...
    if (themeLabel || themePrompt || themeComposition) {
        parts.push(
            [moodDraft ? 'Theme' : 'Shoot theme', themeLabel, themePrompt, themeComposition].filter(Boolean).join(': ')
        );
    }
    if (hasSceneImage) {
    ...
    parts.push(promoPortraitStyling.buildPortraitCommercialPrincipleZh(
        o.stylingMode || o.portrait_styling_mode,
        o.themeKey
    ));
    if (origUserPrompt) {
        parts.push('Styling and details: ' + origUserPrompt);
    } else {
        const poseLine = promoPortraitStyling.buildPortraitPoseBySceneGeminiLine(o.themeKey);
        if (poseLine) parts.push(poseLine);
    }
    if (shotBrief && !moodDraft) parts.push('This shot variation: ' + shotBrief);
    ...
    parts.push(
        ...
        + 'Do not place any caption, filename, clock, or pixel-size text in the corners, along the top, or along the bottom.'
    );
...
async function ensurePromoSpaceOutputDimensions(buffer, width, height, opts) {
    ...
    const allowUpscale = !(opts && opts.allowUpscale === false);
    if (!allowUpscale) {
        const nativeLong = Math.max(before.width, before.height);
        const targetLong = Math.max(tw, th);
        if (nativeLong < targetLong) {
            console.log(
                '[promo-space] skip upscale',
                before.width + 'x' + before.height,
                'target',
                tw + 'x' + th,
                'tier',
                clamped.space_resolution_tier
            );
            return buffer;
        }
        if (before.width === tw && before.height === th) {
            return buffer;
        }
    }
