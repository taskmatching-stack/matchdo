 ...
 * - 情境圖：status=success、有 result_image_url；source_type 為 custom_product 或 vendor_asset；show_on_homepage 欄位存在時僅 true
 * - 一律 created_at DESC
 *
...
const PROMO_SCENE_MEDIA_WALL_SELECT = 'id, user_id, source_type, source_id, source_image_url, aspect_ratio, width, height, user_prompt, scene_template_key, scene_key, final_prompt, result_image_url, ai_tags, image_semantics_json, description, semantics_generated_at, created_at, show_on_homepage';
...
function buildPromoSceneMediaWallQuery(supabase, selectCols, opts) {
    ...
    let q = supabase.from('product_promo_generations').select(selectCols)
        .eq('status', 'success')
        .in('source_type', ['custom_product', 'vendor_asset'])
...
async function fetchPromoSceneMediaWallRows(supabase, rangeFrom, rangeTo, logFn) {
    ...
    // 先不篩選 show_on_homepage，有欄位才篩選
    let withHomepage = false;
    let res = await orderRange(PROMO_SCENE_MEDIA_WALL_SELECT, withHomepage);
    // 如果有 show_on_homepage 欄位且沒其他錯誤，改為篩選 true
    if (!res.error) {
        // 有欄位就用篩選，讓媒體牆只顯示公開的
        withHomepage = true;
        res = await orderRange(PROMO_SCENE_MEDIA_WALL_SELECT, withHomepage);
        if (res.error && isSupabaseMissingColumnError(res.error, 'show_on_homepage')) {
            // 欄位不存在，回到不篩選版本
            withHomepage = false;
            res = await orderRange(PROMO_SCENE_MEDIA_WALL_SELECT, false);
        }
    }
...
async function fetchPromoSceneMediaWallPool(supabase, limit, logFn) {
    ...
    if (!res.error) {
        withHomepage = true;
        res = await orderLimit(PROMO_SCENE_MEDIA_WALL_SELECT, withHomepage);
        if (res.error && isSupabaseMissingColumnError(res.error, 'show_on_homepage')) {
            withHomepage = false;
            res = await orderLimit(PROMO_SCENE_MEDIA_WALL_SELECT, false);
        }
    }
    if (res.error && isSupabaseMissingColumnError(res.error, 'ai_tags')) {
