...
function buildCustomProductMediaWallQuery(supabase, selectCols, filters) {
    const { categoryKeysToMatch, filterCategoryKey, filterSubcategoryKey, withHomepageFilter, searchPattern, excludeSoftDeleted } = filters || {};
    let q = supabase.from('custom_products').select(selectCols)
        .not('ai_generated_image_url', 'eq', null);
    if (excludeSoftDeleted !== false) {
        q = q.is('soft_deleted_at', null);
    }
...
function buildPromoSceneMediaWallQuery(supabase, selectCols, opts) {
    const { withHomepageFilter, excludeSoftDeleted } = opts || {};
...
async function fetchCustomProductMediaWallRows(supabase, filters, rangeFrom, rangeTo, logFn) {
    const orderRange = (selectCols, homepageFilter, excludeSoftDeleted) =>
        buildCustomProductMediaWallQuery(supabase, selectCols, {
            ...filters,
            withHomepageFilter: homepageFilter !== false,
            excludeSoftDeleted: excludeSoftDeleted !== false
        })
    ...
    if (isSupabaseMissingColumnError(res.error, 'soft_deleted_at')) {
        res = await orderRange(CUSTOM_PRODUCT_MEDIA_WALL_SELECT, true, false);
        if (res.error && isSupabaseMissingColumnError(res.error, 'title_en')) {
            res = await orderRange(CUSTOM_PRODUCT_MEDIA_WALL_SELECT_NO_TITLE_EN, true, false);
        }
        if (!res.error) return res.data || [];
    }
...
async function fetchPromoSceneMediaWallRows(supabase, rangeFrom, rangeTo, logFn) {
    const orderRange = (selectCols, withHomepageFilter, excludeSoftDeleted) =>
        buildPromoSceneMediaWallQuery(supabase, selectCols, {
            withHomepageFilter,
            excludeSoftDeleted: excludeSoftDeleted !== false
        })
            .order('created_at', { ascending: false })
    ...
    if (res.error && isSupabaseMissingColumnError(res.error, 'soft_deleted_at')) {
        res = await orderRange(PROMO_SCENE_MEDIA_WALL_SELECT, withHomepage, false);
        if (res.error && isSupabaseMissingColumnError(res.error, 'generation_meta_json')) {
            res = await orderRange(PROMO_SCENE_MEDIA_WALL_SELECT_NO_META, withHomepage, false);
        }
    }
