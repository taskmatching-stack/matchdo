...
function buildVendorStylesHtml(opts) {
    ...
    const title = catObj && catObj.name
        ...
        : '廠商版型 - MATCHDO 合做';
    const desc = '瀏覽公開廠商數位原型。點「用此款進行設計」帶入設計稿；有搭配材料／配件時可「看可搭配」。';
    ...
    const cards = items.map(function (item) {
        ...
        const guide = matchGuidePath(item);
        ...
        // 與設計頁舊版卡一致：主鈕「用此款進行設計」＋「看可搭配」→ 產品樹（勿用「作品頁」連 inspiration）
        const guideBtn = '<a class="btn btn-sm btn-outline-secondary" href="' + escapeHtmlAttr(guide) + '">看可搭配</a>';
        const imgHtml = img
            ? '<a href="' + escapeHtmlAttr(design) + '" class="vs-card-img-link"><img src="' + escapeHtmlAttr(img) + '" alt="' + escapeHtmlAttr(item.title || '') + '" loading="lazy" width="400" height="400"></a>'
    ...
    return '<!DOCTYPE html>\n' +
        ...
        '<p class="vs-lead">公開廠商數位原型庫。點「用此款進行設計」帶入參考圖；「看可搭配」開啟產品樹（材料／配件）。</p>\n' +
