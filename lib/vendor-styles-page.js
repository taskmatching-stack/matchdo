...
function designPath(item) {
    ...
    let url = '/custom-product.html?tab=product-design&prototype_asset_id=' + encodeURIComponent(item.id);
...
function buildVendorStylesHtml(opts) {
    ...
    const title = catObj && catObj.name
        ...
        : '廠商版型 - MATCHDO 合做';
    const desc = '瀏覽公開廠商數位原型。點進獨立作品頁，或以「用此款進行設計」帶入設計稿；亦可查看廠商主頁。';
    ...
    const cards = items.map(function (item) {
        ...
        const imgHtml = img
            ? '<a href="' + escapeHtmlAttr(insp) + '" class="vs-card-img-link"><img src="' + escapeHtmlAttr(img) + '" alt="' + escapeHtmlAttr(item.title || '') + '" loading="lazy" width="400" height="400"></a>'
        ...
        return (
            ...
            '<h2 class="vs-card-title"><a href="' + escapeHtmlAttr(insp) + '">' + t + '</a></h2>' +
    ...
    return '<!DOCTYPE html>\n' +
        ...
        '<p class="vs-lead">公開廠商數位原型庫。點「作品頁」看獨立說明；「用此款進行設計」進入設計稿工具。</p>\n' +
