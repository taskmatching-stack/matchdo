...
/** 生圖原則：三種衣著都只複製人物＋忽略姿勢；依原圖另鎖服裝來源（英文關鍵指令） */
function buildPortraitCommercialPrincipleZh(mode, themeKey) {
    const m = normalizePortraitStylingMode(mode);
    if (m === 'reference') {
        return 'Generation rules (must follow): copy the person only; keep the garment type, color palette, and silhouette from the reference; ***Ignore the original pose***, ***no erotic tone***.';
    }
