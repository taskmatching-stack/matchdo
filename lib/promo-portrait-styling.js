...
/**
 * Grok 易把 bokeh／光圈片數字面畫成漂浮光球；送 Grok 前轉寫攝影參數英文句。
 */
function sanitizePromoCameraBlockForGrok(cameraBlock) {
    let s = String(cameraBlock || '').trim();
    if (!s) return '';
    s = s.replace(/\b(?:five|seven|nine|eleven)[-\s]?blade\s+aperture\s+iris[^.]*\.?/gi,
        'aperture rendering as background optical blur only, not visible shapes in the scene.');
    s = s.replace(/\b(?:pentagonal|heptagonal|round|smooth|swirly|creamy|organic)\s+bokeh[^.]*\.?/gi,
        'background optical blur only (not physical shapes in the scene).');
    s = s.replace(/\bbokeh\s+highlights?[^.]*\.?/gi, 'background blur only.');
    s = s.replace(/\bbokeh\s+(?:falloff|circles?|tendency)[^.]*\.?/gi, 'background blur only.');
    s = s.replace(/\bbokeh\b/gi, 'background blur');
    return s.replace(/\s{2,}/g, ' ').replace(/\.\s*\./g, '.').trim();
}

function buildPortraitGrokOpticsNegativeGuard() {
    return (
        'Optics (mandatory): depth of field and background blur are lens effects only—'
        + 'do NOT add soap bubbles, floating orbs, spheres, bubble overlays, lens-shaped objects, '
        + 'or bokeh highlight shapes as physical elements. '
