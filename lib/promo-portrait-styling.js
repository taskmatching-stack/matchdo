...
/** 400 重試：保持原衣著模式，只小幅微調衣著尺度（不切換依場景／依參考圖／依描述） */
function buildPortraitBlockRetryUserPrompt(userPrompt, originalMode) {
    ...
    let hint;
    if (m === 'reference') {
        hint = '維持原風格與款式色系，僅小幅微調衣著尺度以符合商業人像審核，不要大改服裝。';
    } else if (m === 'prompt') {
        hint = '維持描述風格，僅小幅微調衣著尺度，不要大改服裝。';
    } else {
        hint = '維持原風格，僅小幅微調衣著尺度，不要大改服裝。';
    }
...
function buildPortraitStylingGeminiLead(mode, opts) {
    ...
    const m = normalizePortraitStylingMode(mode);
    if (m === 'reference') {
        if (o.blockRetryClothingAdjust) {
            return 'Preserve the same person\'s facial identity and likeness from reference image 1. '
                + 'Keep garment type, color palette, and overall outfit from reference image 1; '
                + 'slightly adjust coverage only as needed for commercial portrait standards; '
                + 'do not invent or replace with a different outfit. '
                + 'User description may adjust hairstyle, expression, and small props only—not the main outfit silhouette.';
        }
        return 'Preserve the same person\'s facial identity and likeness from reference image 1. '
...
function buildPortraitStylingMoodFaceLines(mode, userPrompt, opts) {
    ...
    if (m === 'reference') {
        if (o.blockRetryClothingAdjust) {
            lines.push('衣服以第一張上傳圖為主（款式色系維持），僅小幅微調衣著尺度，不要改成完全不同套。');
        } else {
    ...
    if (o.blockRetryClothingAdjust === true && m !== 'reference') {
