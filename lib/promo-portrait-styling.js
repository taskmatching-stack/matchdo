...
/** 依參考圖 lead：生圖原則（送給模型，含「不要情色感」） */
function buildPortraitReferenceCommercialFramingGeminiLine() {
    return 'Required generation principle: commercial lifestyle portrait. 不要情色感. Keep the same garment.';
}

/** 人像共通生圖原則（三種衣著都送給模型） */
function buildPortraitCommercialPrincipleZh(mode) {
    const core = '生圖原則（必須遵守）：商用生活人像，不要情色感';
...
function buildPortraitStylingMoodSwapClosing(mode) {
    ...
    if (m === 'reference') {
        return '成品：人與體型＝第一張；衣服（款式色系）＝第一張；構圖商用，不要情色感；場景＝第二張。' + blend;
    }
    if (m === 'scene') {
