...
/** 參考圖只當人物／衣著；***忽略原圖姿勢***（證件照除外） */
function buildPortraitIgnoreRefPoseEmphasisLine(themeKey) {
    if (String(themeKey || '').trim() === 'portrait_formal_id') return '';
    return '***忽略原圖姿勢***。參考圖1只提供人物與衣著，禁止沿用原圖站姿、坐姿、躺姿或手勢。***姿勢依場景***。***不要情色感***。';
}

/** 未填描述時：姿勢依場景（證件照除外） */
function buildPortraitPoseBySceneGeminiLine(themeKey) {
    if (String(themeKey || '').trim() === 'portrait_formal_id') return '';
    return '***忽略原圖姿勢***，***姿勢依場景***（可站可坐可倚靠）。';
}

/** 依參考圖 lead */
function buildPortraitReferenceCommercialFramingGeminiLine() {
    return 'Reference image 1 is person and garment only. ***忽略原圖姿勢***. ***姿勢依場景***. ***不要情色感***.';
}

/** 人像共通生圖原則（三種衣著都送給模型；證件照姿勢維持參考圖） */
function buildPortraitCommercialPrincipleZh(mode, themeKey) {
    const m = normalizePortraitStylingMode(mode);
    const isFormalId = String(themeKey || '').trim() === 'portrait_formal_id';
    if (isFormalId) {
        if (m === 'reference') {
            return '生圖原則（必須遵守）：只複製人物與衣著，姿勢維持參考圖，***不要情色感***。';
        }
        return '生圖原則（必須遵守）：只複製人物，姿勢維持參考圖，***不要情色感***。';
    }
    if (m === 'reference') {
        return '生圖原則（必須遵守）：只複製人物與衣著，***忽略原圖姿勢***，***姿勢依場景***，***不要情色感***。';
    }
    return '生圖原則（必須遵守）：只複製人物，***忽略原圖姿勢***，***姿勢依場景***，***不要情色感***。';
}
...
function buildPortraitReferenceGarmentGeminiLine() {
    return 'Keep the main garment outfit from reference image 1 (garment type, color palette, silhouette); do not invent or swap the core clothing. '
        + 'Copy person and clothing only. ***忽略原圖姿勢***. '
...
function buildPortraitStylingGeminiLead(mode, themeKey) {
    const m = normalizePortraitStylingMode(mode);
    const isFormalId = String(themeKey || '').trim() === 'portrait_formal_id';
    const markers = isFormalId
        ? 'Keep the reference pose. ***不要情色感***.'
        : '***忽略原圖姿勢***. ***姿勢依場景***. ***不要情色感***.';
    if (m === 'reference') {
        return 'Preserve the same person\'s facial identity and likeness from reference image 1. '
            + (isFormalId
                ? ('Keep the main garment outfit from reference image 1 (garment type, color palette, silhouette); do not invent or swap the core clothing. '
                    + 'Copy person and clothing only. ' + markers + ' '
                    + 'User description may adjust hairstyle and expression—not the main garment.')
...
function buildPortraitStylingMoodSwapClosing(mode) {
    ...
    if (m === 'reference') {
        return '成品：人與體型＝第一張；衣服（款式色系）＝第一張；***忽略原圖姿勢***；***姿勢依場景***；構圖商用，***不要情色感***；場景＝第二張。' + blend;
    }
    if (m === 'scene') {
        return '成品：人與體型＝第一張；衣服依主題與場景；***忽略原圖姿勢***；***姿勢依場景***；構圖商用，***不要情色感***；場景＝第二張。' + blend;
    }
    return '成品：人與體型＝第一張；衣服依描述；***忽略原圖姿勢***；***姿勢依場景***；構圖商用，***不要情色感***；場景＝第二張。' + blend;
}
