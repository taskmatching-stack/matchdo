...
/** 依參考圖：商用構圖原則（英文；正面陳述，不寫 erotic／Do not） */
function buildPortraitReferenceCommercialFramingGeminiLine() {
    return 'Required generation principle: a commercial lifestyle portrait with tasteful brand-safe framing and a clean editorial mood, keeping the same garment.';
}

/** 人像共通生圖原則（中文；三種衣著都要；正面陳述） */
function buildPortraitCommercialPrincipleZh(mode) {
    const core = '生圖原則（必須遵守）：構圖為商用生活人像，得體、品牌安全，非情色風格';
    if (normalizePortraitStylingMode(mode) === 'reference') {
        return core + '，服裝款式色系維持上傳圖。';
    }
    return core + '。';
}
...
function buildPortraitStylingMoodSwapClosing(mode) {
    ...
    if (m === 'reference') {
        return '成品：人與體型＝第一張；衣服（款式色系）＝第一張；構圖必須商用生活人像、得體非情色；場景＝第二張。' + blend;
    }
    if (m === 'scene') {
        return '成品：人與體型＝第一張；衣服依主題與場景；構圖必須商用生活人像、得體非情色；場景＝第二張。' + blend;
    }
    return '成品：人與體型＝第一張；衣服依描述；構圖必須商用生活人像、得體非情色；場景＝第二張。' + blend;
}
