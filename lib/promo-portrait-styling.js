...
function buildPortraitCommercialPrincipleZh(mode, themeKey) {
    ...
    if (m === 'reference') {
        return '生圖原則（必須遵守）：只複製人物與衣著，***忽略原圖姿勢***，***不要情色感***。';
    }
...
function buildPortraitStylingMoodSwapClosing(mode) {
    ...
    if (m === 'reference') {
        return '成品：人與體型＝第一張；衣服（款式色系）＝第一張；***忽略原圖姿勢***；構圖商用，***不要情色感***；場景＝第二張。' + blend;
    }
    if (m === 'scene') {
