...
const GROK_CAMERA_OPTICS_LITE =
    'Depth of field as soft background blur only.';

function buildPortraitNoLensFlareGuard() {
    return (
        'Do not add or keep lens flare, sun stars, anamorphic streaks, ghosting, or light leaks as overlays. '
        + 'Remove sparkle layers that are not real reflections on surfaces.'
    );
}

/** 光圈葉片決定失焦高光形狀；Grok 易忽略設定、改鋪預設多邊形。改寫成「形狀＋少量＋跟光」。 */
function rewriteGrokApertureBladeClause(clause) {
    const t = String(clause || '');
    let shape = 'soft round out-of-focus speculars matching the selected aperture blade count';
    if (/\bfive[-\s]?blade\b/i.test(t) || /\bpentagonal\b/i.test(t)) {
        shape = 'sparse pentagonal out-of-focus speculars (5-blade iris)';
    } else if (/\bseven[-\s]?blade\b/i.test(t) || /\bheptagonal\b/i.test(t)) {
        shape = 'sparse heptagonal out-of-focus speculars (7-blade iris)';
    } else if (/\bnine[-\s]?blade\b/i.test(t) || /\bnonagonal\b/i.test(t)) {
        shape = 'sparse nearly-round out-of-focus speculars (9-blade iris)';
    } else if (/\beleven[-\s]?blade\b/i.test(t) || /\bhendecagonal\b/i.test(t)) {
        shape = 'sparse round out-of-focus speculars (11-blade iris)';
    } else if (/\bsix[-\s]?blade\b/i.test(t) || /\bhexagonal\b/i.test(t)) {
        shape = 'sparse hexagonal out-of-focus speculars (6-blade iris)';
    }
    return (
        shape
        + ' only, a few small low-opacity discs from real bright sources already in the scene, '
        + 'placed in defocused background according to the light direction—not a full-frame overlay'
    );
}

function buildPortraitGrokBokehRestraint() {
    return (
        'Aperture-iris highlights: keep the highlight shape from the selected blade count '
        + '(5=pentagon, 7=heptagon, 9–11=rounder). Do not substitute a different polygon. '
        + 'They may appear only as a few subtle out-of-focus speculars from real lights already in the plate '
        + '(window, lamps, specular catchlights), along the light path, in defocused air. '
        + 'Not many, not obvious, not a glitter field across the in-focus person, bed, floor, or walls. '
        + 'If the source scene has no point lights in the blur zone, do not invent extra discs.'
    );
}

/** 單句攝影參數：剝 Grok 易鋪滿的耀光用語；葉片句改寫成跟光的少量高光，不刪形狀。 */
function stripGrokHeavyOpticsClause(clause) {
    let t = String(clause || '').trim();
    if (!t) return '';

    if (
        /\b(?:five|six|seven|nine|eleven)[-\s]?blade\b/i.test(t)
        || /\baperture\s+iris\b/i.test(t)
        || /\b(?:pentagonal|hexagonal|heptagonal|nonagonal|hendecagonal)\s+bokeh\b/i.test(t)
    ) {
        return rewriteGrokApertureBladeClause(t);
    }
    ...
    t = t.replace(/\b(?:lens\s+)?light\s+spots?\b/gi, '');
    t = t.replace(/\bsparkle(?:s| overlays?)?\b/gi, '');
    t = t.replace(/\bfloating\s+(?:bright\s+)?orbs?\b/gi, '');
...
/**
 * 人像專用：相機 DB 仍是產品向 fragment，這裡只改「送進人像 prompt 的字」。
 * 拿掉會抵銷 EV 的產品句（no lighting change／product photo），
 * 暗角不壓整張曝光；EV 當整張測光，不加打亮人物。
 */
function detectPromoPortraitExposureEvStops(text) {
    const s = String(text || '');
    const plus = s.match(/\bEV\s*\+\s*([12])\b/i);
    if (plus) return Number(plus[1]);
    const minus = s.match(/\bEV\s*[-−]\s*([12])\b/i);
    if (minus) return -Number(minus[1]);
    if (/\bEV\s*0\b/i.test(s)) return 0;
    if (/\bincreased by two stops\b/i.test(s)) return 2;
    if (/\bincreased by one stop\b/i.test(s)) return 1;
    if (/\bdecreased by two stops\b/i.test(s) || /\btwo stops under\b/i.test(s)) return -2;
    if (/\bdecreased by one stop\b/i.test(s) || /\bone stop under\b/i.test(s)) return -1;
    return null;
}

function tidyPromoPortraitCameraBlockText(s) {
    let t = String(s || '');
    t = t.replace(/\s*,\s*,+/g, ',');
    t = t.replace(/,\s*\./g, '.');
    t = t.replace(/\.\s*\./g, '.');
...
function stripOnePromoPortraitCameraClause(clause) {
    let s = String(clause || '');
    s = s.replace(/\bno\s+scene\s+or\s+lighting\s+change\b[,;.]?/gi, '');
    s = s.replace(/\bwithout\s+scene\s+or\s+lighting\s+change\b[,;.]?/gi, '');
    s = s.replace(/\bwithout\s+changing\s+(?:the\s+)?scene\s+or\s+lighting(?:\s+change)?\b[,;.]?/gi, '');
    s = s.replace(/\bwithout\s+changing\s+(?:the\s+)?scene\b[,;.]?/gi, '');
    s = s.replace(/\bphotorealistic\s+product\s+photos?\b[,;.]?/gi, '');
    s = s.replace(/\bcommercial\s+product\s+advertising\b/gi, 'commercial photography');
    s = s.replace(/\bon\s+bright\s+reflective\s+product\s+surfaces\b/gi, '');
    s = s.replace(/\bon\s+product\s+surfaces\b/gi, '');
    s = s.replace(/\bon\s+product\s+edges\s+and\s+materials\b/gi, '');
    s = s.replace(/\bstrong\s+optical\s+vignetting\s+darkening\s+the\s+corners\b/gi,
        'mild corner falloff from the lens only, without lowering overall exposure');
    s = s.replace(/\boptical\s+vignetting\s+darkening\s+the\s+corners\b/gi,
        'mild corner falloff from the lens only, without lowering overall exposure');
    return tidyPromoPortraitCameraBlockText(s);
}

function stripPromoPortraitProductLightingLocks(text) {
    return String(text || '')
        .split(/\.\s+/)
        .map(stripOnePromoPortraitCameraClause)
        .filter(Boolean)
        .join('. ');
}

function buildPortraitWholeFrameExposureCloser(cameraBlock) {
    const stops = detectPromoPortraitExposureEvStops(cameraBlock);
    if (stops == null) return '';
    const base =
        'Camera exposure compensation is a whole-frame meter setting: it applies to the entire photograph '
        + '(environment and subject together), like changing shutter/ISO—not a separate light on the person. '
        + 'Color and atmosphere language must not override this EV for overall brightness.';
    if (stops > 0) {
        return base + ' Positive EV must read as a brighter overall plate by the stated stops.';
    }
    if (stops < 0) {
        return base + ' Negative EV must read as a darker overall plate by the stated stops.';
    }
    return base;
}

/** 人像 FLUX 易把相機句畫成整張灰霧；與氛圍第二段同一組防霧句。 */
function buildPortraitFluxDehazeGuard() {
    return [
        '畫面要清晰透亮、對比乾淨，不要整體霧化、柔焦蒙霧、灰霧或低對比發灰。',
        '保留人物與環境細節與邊緣銳度，不要柔糊一片。',
        'No heavy haze, no soft-focus veil, no low-contrast gray wash.'
    ].join(' ');
}

function applyPortraitFluxDehazeGuard(promptText) {
    const p = String(promptText || '').trim();
    if (!p) return p;
    if (/不要整體霧化/.test(p) || /no heavy haze/i.test(p)) return p;
    return buildPortraitFluxDehazeGuard() + ' ' + p;
}

function sanitizePromoCameraBlockForPortrait(cameraBlock) {
    let s = stripPromoPortraitProductLightingLocks(cameraBlock);
    if (!s) return '';
    if (/whole-frame meter setting/i.test(s)) return s;
    const closer = buildPortraitWholeFrameExposureCloser(s);
    if (closer) {
        s = tidyPromoPortraitCameraBlockText(s.replace(/[.]+$/, '')) + '. ' + closer;
    }
    return s;
}
...
function sanitizePromoCameraBlockForGrok(cameraBlock) {
    ...
    if (/\b(?:lens|optical\s+character|vintage|prime|telephoto)\b/i.test(s)) {
        s += ' No added lens flare, ghosting, or sparkle overlays.';
    }
    if (!/do not add or keep lens flare/i.test(s)) {
        s += ' ' + buildPortraitNoLensFlareGuard();
    }
    if (!/Aperture-iris highlights/i.test(s)) {
        s += ' ' + buildPortraitGrokBokehRestraint();
    }
...
function buildPortraitGrokOpticsNegativeGuard() {
    return (
        'Optics (mandatory): depth of field is background softness only. '
        + buildPortraitNoLensFlareGuard() + ' '
        + buildPortraitGrokBokehRestraint() + ' '
 ...
 */
function buildPortraitStylingGrokSwapLead(mode) {
    const m = normalizePortraitStylingMode(mode);
    const lighting = m === 'reference'
        ? (
            'Lighting and shadow on the person must preserve <IMAGE_1> reference rendering: keep the same key-to-fill ratio, highlight placement on skin and garment folds, shadow depth on the outfit, and overall contrast character from the upload—do not flatten, wash out, or re-light the subject into a different lighting style. '
            + 'Ground the body in <IMAGE_0> with believable contact shadows and ambient occlusion at feet, seats, and props; edge rim from <IMAGE_0> may kiss silhouettes only where consistent with the scene, but the dominant light-and-shadow read on face, skin, hair, and clothing stays true to <IMAGE_1>. '
        )
        : (
    ...
    return (
        ...
        + 'Edge transitions must follow <IMAGE_0> sharpness and ambient color on silhouettes, and rim light from the scene—never a generic blur halo unrelated to the environment. '
...
function buildPortraitStylingGrokSwapClosing(mode) {
    ...
    const blend = m === 'reference'
        ? (
            ...
            + 'Preserve <IMAGE_1> light-and-shadow rendering on the person: highlights, shadow placement on garment folds, skin tone under the same lighting, and outfit contrast must read like the reference photo—not relit flat or mismatched to the upload. '
            + 'Match perspective and contact shadows to <IMAGE_0>; edge transitions must match <IMAGE_0> depth of field and atmosphere—not generic feathering; no hard cut-and-paste. '
            + 'The person must not look pasted, brighter, or flatter than the scene.'
        )
        : (
            'Place a complete person into <IMAGE_0>, ***Ignore the original pose***; <IMAGE_1> is face and body reference only—do not keep its crop box, rectangular border, sticker, or layers. '
...
function buildPortraitGrokSwapCaptions(stylingMode) {
    return {
        ...
        lead: buildPortraitStylingGrokSwapLead(stylingMode),
...
module.exports = {
    ...
    sanitizePromoCameraBlockForPortrait,
    buildPortraitFluxDehazeGuard,
    applyPortraitFluxDehazeGuard,
