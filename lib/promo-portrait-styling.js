...
function buildPortraitStylingHybridSwapLead() {
    return 'Single unified commercial photograph. Reference image 1 is identity only (face, body type, likeness). '
        + 'Do not paste, overlay, or keep the rectangular crop of image 1. '
        + 'Re-render one complete standing person inside reference image 2; legs and feet on the ground plane. '
        + 'Match lighting, shadows, color grade, and edges to image 2.';
}

function buildPortraitStylingMoodSwapClosing(mode) {
    const m = normalizePortraitStylingMode(mode);
    const blend = '站位對齊第二張留白區；在第二張裡重繪完整站立人物（含腿腳），腳踩在地面材質上；'
        + '第一張只作臉與身材參考，禁止保留裁切框、矩形邊界、貼紙或分層；'
        + '接觸陰影、環境反光、色溫與場景一致；邊緣自然過渡。';
    if (m === 'reference') {
