...
/** 寬鬆尺度 Grok：姿勢與鏡位不跟參考圖（通用）。 */
function buildPortraitExperimentGrokPoseFreedomLine() {
    return (
        'Pose, camera angle, distance, and framing must not mirror the reference image. '
        + 'Invent a new stance and viewpoint that fit the scene brief—not a minor adjustment of the upload.'
    );
}

/** 寬鬆尺度 Grok：配件整合與光影過渡（通用）。 */
function buildPortraitExperimentGrokLightingPropsLine() {
    return (
        'Props and accessories must be physically integrated with correct grip, scale, perspective, and contact shadows; '
        + 'no floating or pasted elements. '
        + 'Person lighting must match the scene with smooth natural shadow transitions; '
        + 'no cutout edges or flat composite look.'
    );
}
...
function buildPromoPortraitExperimentGrokPrompt(opts) {
    ...
    parts.push(buildPortraitExperimentGrokPoseFreedomLine());
    if (hasSceneText) {
        parts.push(
    ...
    parts.push(buildPortraitExperimentGrokLightingPropsLine());
    parts.push('No text, labels, logos, or watermarks in the image.');
    parts.push('Scene-consistent natural lighting throughout the frame. No artificial vignette or darkened corners.');
...
module.exports = {
    ...
    buildPortraitExperimentGrokPoseFreedomLine,
    buildPortraitExperimentGrokLightingPropsLine,
