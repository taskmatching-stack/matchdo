...
/** 依參考圖：商用構圖，避免情色／挑逗感（英文） */
function buildPortraitReferenceNonEroticCompositionGeminiLine() {
    return 'Commercial lifestyle portrait only: natural relaxed posing, tasteful framing, and a non-sexualized mood. '
        + 'Do not copy erotic, fetish, or suggestive composition from reference image 1 '
        + '(no provocative poses, overt cleavage emphasis, or fetish staging); '
        + 'reframe as a clean brand-safe portrait while keeping the same garment.';
}

/** 依參考圖：商用構圖，避免情色／挑逗感（中文，氛圍／混合貼人） */
function buildPortraitReferenceNonEroticCompositionMoodLine() {
    return '構圖與姿勢走商業生活人像：自然、得體，避免情色或過度挑逗感；'
        + '不要照搬參考圖的煽情構圖或 fetish 情境，服裝款式色系仍維持上傳圖。';
}
...
function buildPortraitReferenceGarmentNotPropsGeminiLine() {
    return 'Keep the main garment outfit from reference image 1 (garment type, color palette, silhouette); do not invent or swap the core clothing. '
        ...
        + buildPortraitReferenceNonEroticCompositionGeminiLine() + ' '
...
function buildPortraitStylingMoodSwapClosing(mode) {
    ...
    if (m === 'reference') {
        return '成品：人與體型＝第一張；衣服（款式色系）＝第一張；道具不抄第一張；構圖避免情色挑逗感；場景＝第二張。' + blend;
    }
    if (m === 'scene') {
...
function buildPortraitStylingMoodFaceLines(mode, userPrompt, opts) {
    ...
    if (m === 'reference') {
        ...
        lines.push(buildPortraitReferenceNonEroticCompositionMoodLine());
    } else if (m === 'scene') {
