...
/**
 * 實驗模式給 FLUX 的開頭：像素會鎖姿勢、長提示詞會重畫人。
 * 不指定站／坐／倚靠，只要求換姿勢、同一個人。
 */
function buildPortraitFluxExperimentPriorityLead(mode) {
    ...
    return (
        ...
        + '***Do not copy the pose*** from the reference: do not reuse its limb positions or body stance; choose a new pose that fits the scene. '
        + clothes
    );
}
