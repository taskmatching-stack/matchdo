...
/** 原圖衣著：商用構圖（英文，僅 reference 清晰／Gemini lead；勿寫 erotic/suggestive，會觸發 400） */
function buildPortraitReferenceCommercialFramingGeminiLine() {
    return 'Commercial lifestyle portrait only: tasteful framing and a brand-safe catalog mood. '
        + 'Reframe as a clean commercial portrait while keeping the same garment.';
}

/** 三模式共用：商用構圖（中文；實測有效句，須進清晰 Gemini，不只進氛圍／混合） */
...
/** 清晰 Gemini：三模式皆在描述／姿勢句之前插入中文商用構圖 */
function buildPortraitCommercialFramingClearGeminiLine(mode) {
    return buildPortraitCommercialFramingMoodLine(mode);
}

/** 原圖衣著：鎖服裝 + 商用構圖（英文，清晰／Gemini） */
...
module.exports = {
    ...
    buildPortraitCommercialFramingClearGeminiLine,
