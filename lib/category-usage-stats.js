...
function emptyPromoSourceCounts() {
    return {
        promo_page: 0,
        promo_camera: 0,
        promo_camera_product: 0,
        promo_camera_space: 0,
        promo_camera_portrait: 0
    };
}
...
function addPromoSourceCounts(target, delta) {
    ...
    target.promo_camera_product += delta.promo_camera_product || 0;
    target.promo_camera_space += delta.promo_camera_space || 0;
    target.promo_camera_portrait += delta.promo_camera_portrait || 0;
}

function parsePromoGenerationMetaJson(raw) {
    if (!raw) return {};
...
function promoCameraShootModeFromRow(row) {
    ...
    if (mode !== 'camera_advanced') return null;
    return String(meta.shoot_mode || 'product').trim().toLowerCase();
}

/** generation_mode=camera_advanced → 商攝導演（依 shoot_mode 分產品／空間／人像）；其餘 → 情境圖頁 */
function mapPromoRowToSourceDelta(row) {
    const mode = String(row && row.generation_mode || '').trim();
    if (mode === 'camera_advanced') {
        const shoot = promoCameraShootModeFromRow(row) || 'product';
        const delta = emptyPromoSourceCounts();
        delta.promo_camera = 1;
        if (shoot === 'space') delta.promo_camera_space = 1;
        else if (shoot === 'portrait') delta.promo_camera_portrait = 1;
        else delta.promo_camera_product = 1;
        return delta;
    }
    const delta = emptyPromoSourceCounts();
    delta.promo_page = 1;
...
function ensurePromoSubBucket(map, subcategoryKey, subNameByKey, subActiveByKey) {
    ...
    if (!map[k]) {
        ...
        if (map[k].promo_camera_product == null) map[k].promo_camera_product = 0;
        if (map[k].promo_camera_space == null) map[k].promo_camera_space = 0;
        if (map[k].promo_camera_portrait == null) map[k].promo_camera_portrait = 0;
        if (subNameByKey && subcategoryKey && subNameByKey[subcategoryKey]) {
            map[k].subcategory_name = subNameByKey[subcategoryKey];
        }
        if (subActiveByKey && subcategoryKey && subActiveByKey[subcategoryKey] !== undefined) {
...
function aggregatePromoCategoryUsage(items, categories, opts) {
    ...
    (items || []).forEach(function (item) {
        ...
        const ck = String(item.category_key || '').trim() || '(無分類)';
        const delta = {
            promo_page: item.promo_page || 0,
            promo_camera: item.promo_camera || 0,
            promo_camera_product: item.promo_camera_product || 0,
            promo_camera_space: item.promo_camera_space || 0,
            promo_camera_portrait: item.promo_camera_portrait || 0
        };
        const bucket = catByKey[ck];
        if (bucket) {
            ...
            const subs = sortSubcategoryRows(
                ...
                b.subSortOrder
                    ...
                    return {
                        ...
                        promo_camera_product: sub.promo_camera_product,
                        promo_camera_space: sub.promo_camera_space,
                        promo_camera_portrait: sub.promo_camera_portrait
                    };
                });
            return {
                category_key: b.category_key,
                ...
                promo_camera_product: b.totals.promo_camera_product,
                promo_camera_space: b.totals.promo_camera_space,
                promo_camera_portrait: b.totals.promo_camera_portrait,
                subcategories: subs
            };
        });
