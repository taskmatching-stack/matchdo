...
function ensureSubBucket(map, subcategoryKey) {
    const k = subKey(subcategoryKey);
    if (!map[k]) {
        map[k] = Object.assign({ subcategory_key: k === '(未指定子分類)' ? null : k, subcategory_name: k }, emptyCounts());
    }
    return map[k];
}

/**
 * @param {Array} assets vendor_assets rows
...
function aggregateVendorAssetCategoryStats(assets, categories, opts) {
    ...
    (assets || []).forEach(function (row) {
        ...
        const delta = assetDelta(row);
        const bucket = catByKey[ck];
        if (bucket) {
            addCounts(bucket.totals, delta);
            const sub = ensureSubBucket(bucket.subcategories, row.subcategory_key);
        ...
        const sub = ensureSubBucket(orphanByKey[ck].subcategories, row.subcategory_key);
        addCounts(sub, delta);
    ...
    const resultCategories = catList
        ...
        .map(function (c) {
            ...
            const subs = Object.keys(b.subcategories).map(function (sk) {
            ...
            return {
                ...
                prototype_records: b.totals.prototype_records,
                prototype_images: b.totals.prototype_images,
                material_records: b.totals.material_records,
                material_images: b.totals.material_images,
                subcategories: subs
            };
        });
