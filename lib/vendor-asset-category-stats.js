...
const categoryUsageStats = require('./category-usage-stats');
...
function emptyCounts() {
    return {
        ...
        official_part_records: 0,
        vendor_part_records: 0,
        official_part_images: 0,
        vendor_part_images: 0
    };
}
...
function addCounts(target, delta) {
    ...
    target.part_records += delta.part_records || 0;
    target.part_images += delta.part_images || 0;
    target.official_part_records += delta.official_part_records || 0;
    target.vendor_part_records += delta.vendor_part_records || 0;
    target.official_part_images += delta.official_part_images || 0;
    target.vendor_part_images += delta.vendor_part_images || 0;
}
...
function assetDelta(row, isOfficial) {
    ...
    if (kind === 'material') {
        ...
        delta.part_records = 1;
        delta.part_images = images;
        if (isOfficial) {
            delta.official_part_records = 1;
            delta.official_part_images = images;
        } else {
            delta.vendor_part_records = 1;
            delta.vendor_part_images = images;
        }
    }
    return delta;
}

function resolveAssetCategoryKeys(row, catIndex) {
    return categoryUsageStats.resolveCustomProductCategory({
        category: row && row.category_key,
        subcategory_key: row && row.subcategory_key
    }, catIndex);
}
...
function pickCountFields(totals) {
    return {
        ...
        part_records: totals.part_records,
        part_images: totals.part_images,
        official_part_records: totals.official_part_records,
        vendor_part_records: totals.vendor_part_records,
        official_part_images: totals.official_part_images,
        vendor_part_images: totals.vendor_part_images
    };
}
...
function aggregateVendorAssetCategoryStats(assets, categories, opts) {
    ...
    const catIndex = categoryUsageStats.buildSubcategoryIndex(catList, includeInactive);
    ...
    (assets || []).forEach(function (row) {
        if (!row) return;
        const resolved = resolveAssetCategoryKeys(row, catIndex);
        const ck = String(resolved.category_key || '').trim() || '(無分類)';
        const sk = resolved.subcategory_key;
        ...
        if (bucket) {
            ...
            const sub = ensureSubBucket(bucket.subcategories, sk, bucket.subNameByKey, bucket.subActiveByKey);
            if (sk && bucket.subNameByKey[sk]) {
                sub.subcategory_name = bucket.subNameByKey[sk];
            }
            addCounts(sub, delta);
            return;
        }
        ...
        const sub = ensureSubBucket(orphanByKey[ck].subcategories, sk, null, null);
        addCounts(sub, delta);
