...
function buildBrowseStyleCardHtml(item, opts) {
    ...
    // 縮圖走 proxy（避免 CDN cookie）；lightbox 圖庫用直連 Supabase URL（與上傳頁一致，切換才快）
    const lightboxItems = browseItemImageItems(item, function (u) { return u; }, base);
    const lightboxUrls = lightboxItems.map(function (it) { return it.url; }).filter(Boolean);
    const thumbProxied = lightboxUrls.length
        ? (proxyImage(lightboxUrls[0], base) || lightboxUrls[0])
        : (proxyImage(imgRaw, base) || imgRaw);
    const imgUrl = escapeHtmlAttr(thumbProxied);
    const imageItemsJson = JSON.stringify(lightboxItems).replace(/"/g, '&quot;');
