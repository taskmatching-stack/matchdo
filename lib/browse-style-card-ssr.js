...
function getCategoryFilterLabel(categories, categoryKey) {
    if (!categoryKey) return '全部';
    var cat = (categories || []).find(function (c) { return c && c.key === categoryKey; });
    return (cat && cat.name) ? cat.name : categoryKey;
}

function buildBrowseFiltersBlockHtml(basePath, categories, categoryKey) {
    var label = getCategoryFilterLabel(categories, categoryKey);
    return (
        ...
        '<span class="dw-browse-filters-toggle-label">' + escapeHtmlText(label) + '</span>' +
        '</span>' +
        '<i class="bi bi-chevron-down dw-browse-filters-toggle-icon" aria-hidden="true"></i>' +
        '</button>' +
        '<div class="dw-browse-filters">' +
        categoryFilterLinks(basePath, categories, categoryKey) +
        '</div></div>'
    );
}
