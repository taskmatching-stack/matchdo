...
function buildVendorStylesHtml(opts) {
    ...
    const countText = lang === 'en'
        ? (String(total) + ' items' + (categoryKey ? ' (filtered)' : ''))
        : ('共 ' + cards.escapeHtmlText(String(total)) + ' 項' + (categoryKey ? '（已篩選分類）' : ''));
    const bodyHtml =
        ...
        cards.buildBrowseFiltersBlockHtml('/vendor-styles/', categories, categoryKey, lang) + '\n' +
        '<p class="design-workspace-meta" data-browse-count="' + cards.escapeHtmlAttr(String(total)) + '"' +
        (categoryKey ? ' data-browse-filtered="1"' : '') + '>' + countText + '</p>\n' +
