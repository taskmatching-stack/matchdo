...
function describeRetentionMilestonesForAdmin(row) {
    if (!row) return null;
    if (row.soft_deleted_at) {
        const softAt = new Date(row.soft_deleted_at);
        const hardAt = new Date(softAt.getTime() + msDays(softDeleteGraceDays()));
        return {
            summary: '已軟刪',
            soft_deleted_at: row.soft_deleted_at,
            hard_delete_estimated_at: hardAt.toISOString()
        };
    }
    const tier = row.retention_tier || 'free';
    if (isRetentionExemptTier(tier)) {
        return { summary: '長期保存（付費／員工）' };
    }
    const anchor = effectiveRetentionAnchor(row);
    if (!anchor) return { summary: '無留存锚點' };
    const coldAt = new Date(anchor.getTime() + msDays(coldlineDays()));
    const softAt = new Date(anchor.getTime() + msDays(coldlineDays() + deleteAfterColdlineDays()));
    if (row.storage_tier === 'coldline') {
        return {
            summary: 'Coldline',
            anchor_at: anchor.toISOString(),
            coldline_since_estimated_at: coldAt.toISOString(),
            soft_delete_estimated_at: softAt.toISOString()
        };
    }
    return {
        summary: 'Standard',
        anchor_at: anchor.toISOString(),
        coldline_estimated_at: coldAt.toISOString(),
        soft_delete_estimated_at: softAt.toISOString()
    };
}
...
module.exports = {
    ...
    describeRetentionMilestonesForAdmin,
    rowIsSoftDeleted,
    coldlineDays,
    wallGraceDays,
