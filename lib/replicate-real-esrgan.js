 ...
 * @param {number} basePoints — payment_config 的 2× 基準（預設 1）
...
async function prepareImageDataUri(buffer, mimetype) {
    ...
    if (mime === 'image/png' || mime === 'image/webp' || mime === 'image/jpeg' || mime === 'image/jpg') {
        ...
        buf = await sharp(buffer, { failOn: 'none' }).rotate().jpeg({ quality: 92, mozjpeg: true }).toBuffer();
        outMime = 'image/jpeg';
    }
 ...
 * @returns {Promise<{ buffer: Buffer, mimetype: string, width: number, height: number, scale: number }>}
 */
async function realEsrganUpscale(buffer, mimetype, apiToken, opts) {
    if (!apiToken) {
        const err = new Error('伺服器未設定 REPLICATE_API_TOKEN');
        err.status = 503;
        throw err;
    }
    ...
    const createRes = await fetch(PREDICT_URL, {
        method: 'POST',
        headers: {
    ...
    let outBuf = Buffer.from(await imgRes.arrayBuffer());
    let outMime = (imgRes.headers.get('content-type') || 'image/jpeg').split(';')[0].trim() || 'image/jpeg';
