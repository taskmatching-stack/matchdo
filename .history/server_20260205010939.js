...
app.post('/api/generate-product-image', express.json(), async (req, res) => {
    try {
        ...
        // 取得圖片資料
        const image = result.response.candidates[0].content.parts.find(part => part.inlineData);
        
        if (!image || !image.inlineData) {
            return res.status(500).json({ success: false, error: 'AI 生成圖片失敗' });
        }

        // 儲存圖片到本地
        const imageBuffer = Buffer.from(image.inlineData.data, 'base64');
        const filename = `generated-${Date.now()}.${image.inlineData.mimeType.split('/')[1]}`;
        ...
        res.status(500).json({ success: false, error: error.message || '生成失敗' });
    }
});
...
app.post('/api/analyze-custom-product', upload.array('images', 10), async (req, res) => {
    try {
        ...
        // AI 分析提示詞
        const analysisPrompt = `你是專業的產品訂製分析師。請分析以下產品圖片，並提供詳細的製作建議。
...
請以 JSON 格式回應：
{
  ...
  "recommendations": "製作建議"
        ...
        // 模擬推薦廠商（實際應查詢資料庫）
        const manufacturers = generateMockManufacturers(category, analysis);
        ...
        const projectData = {
            ...
            image_urls: req.files ? req.files.map(f => `/uploads/${f.filename}`) : [],
            created_at: new Date().toISOString()
        };

        // TODO: 儲存到 custom_products 表
