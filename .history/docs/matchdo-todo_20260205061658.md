...
**預期結果**：用戶系統完整運作，管理員可以管理用戶角色

---

### 📅 階段二：發案者功能（第二週，預計 1-2 天）

**目標**：讓發案者可以完整使用平台（發案 → 查看媒合 → 聯繫專家）

**現況盤點：**
- ✅ `client/my-projects.html` - 已存在（需檢查功能完整性）
- ✅ `client/my-custom-products.html` - 已存在（客製產品列表）
- ✅ `client/custom-product-detail.html` - 已存在（客製產品詳情）
- ❌ `client/dashboard.html` - **缺少**（發案者控制台）
- ❌ `client/project-detail.html` - **缺少**（專案詳情）
- ❌ `client/matched-experts.html` - **缺少**（查看媒合的專家）

#### A. 建立發案者控制台 ⭐⭐⭐
- [ ] 建立 `client/dashboard.html`
  - 統計卡片（我的專案數、待處理媒合、已完成）
  - 我的專案列表（前 5 筆，帶狀態標籤）
  - 快速操作按鈕（新增專案、新增客製產品）
  - 最新媒合專家列表（前 5 筆，帶匹配分數）
  - 快速跳轉連結

#### B. 完善現有頁面 ⭐⭐
- [ ] 檢查並增強 `client/my-projects.html`
  - 確認從 API 正確載入專案列表
  - 篩選功能（狀態、分類、日期）
  - 快速操作（查看詳情、編輯、關閉專案）
  - 空狀態處理（沒有專案時顯示引導）

- [ ] 檢查並增強 `client/my-custom-products.html`
  - 確認從 API 載入客製產品列表
  - 顯示媒合狀態
  - 快速操作（查看詳情、編輯）
...
#### D. 建立媒合專家列表 ⭐⭐⭐
- [ ] 建立 `client/matched-experts.html`
  - 顯示所有媒合到的專家（跨所有專案）
  - 專家資訊卡片（照片、評分、經驗、專長）
  - 匹配分數與原因
  - 聯絡狀態標籤（未申請/審核中/已解鎖/已拒絕）
  - 申請查看聯絡方式按鈕
  - 篩選功能（依專案、依分數、依狀態）

**預期結果**：發案者可以發案、查看專案、看到媒合的專家（但還不能聯繫）

---

### 📅 階段三：媒合與聯繫核心（第三週，預計 2-3 天）

**目標**：實現平台核心價值「AI 媒合 + 聯絡資訊解鎖」
