...
# 設定你的帳號（liutsaiiu@gmail.com）為 admin
```

**步驟 3：建立聯絡資訊表** ⭐⭐⭐
```bash
# File: docs/contact-info-schema.sql
# 建立 contact_info 表（詳細聯絡資訊 + 隱私控制）
# ⚠️ 必須在 contact_unlocks 之前執行！
```
...
# 建立 unlocked_contacts VIEW（已解鎖的聯絡資訊檢視）
# ⚠️ 依賴 contact_info 和 matches 表！
```
...
# 包含未讀計數函數、批量標記已讀函數
# ⚠️ 依賴 matches 和 contact_unlocks 表！
```
...
profiles (無依賴)
    ↓
contact_info (依賴 users)
    ↓
matches (依賴 projects, expert_listings)
    ↓
project_items (依賴 projects) ← 統包/分包系統
    ↓
contact_unlocks (依賴 matches, contact_info)
    ↓
...
- [ ] 建立 `client/project-items.html`（分包項目管理）
  ...
  - 編輯/刪除分包項目
  - **儲存後可個別發包**（不需一次全發）
  - 每個分項的發包狀態追蹤（未發包/已發包/已媒合/已完成）
  - 批量操作（全部發包、取消發包）

- [ ] 檢查 `client/my-projects.html`
  - 確認能正確載入專案列表
  - **顯示專案類型標籤**（統包/分包）
  - **分包專案顯示分項進度**（已發包 X/總計 Y 項）
  - 確認能跳轉到詳情頁
...
- [ ] 實作媒合 API `/api/match/run`
  ...
  - **判斷專案類型（統包 or 分包）**
  - 查詢所有專家報價
  - **統包模式**：計算每個專家能滿足的項目數，優先推薦能承接最多項目的廠商
  - **分包模式**：針對每個分項分別媒合對應專家
  - 計算匹配分數（關鍵字、預算、地區、經驗）
  - 儲存媒合記錄到 matches 表
  - 觸發通知（雙方）

- [ ] **實作統包媒合算法** `/api/match/run-turnkey`
  - 讀取專案所有分包項目（project_items）
  - 查詢所有專家的服務範圍（expert_listings）
  - 計算每個專家能承接的項目數量
  - 按「滿足項目數」排序（滿足越多項目排名越前）
  - 同時滿足項目數相同時，比較總評分
  - 回傳前 10 名統包候選廠商
