...
CREATE TABLE IF NOT EXISTS public.matches (
    ...
    project_id uuid,  -- 暫時移除外鍵約束，稍後再加
    expert_listing_id uuid,  -- 暫時移除外鍵約束，稍後再加
...
BEGIN
    ...
    -- 加入 expert_listing_id 外鍵
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'expert_listings') THEN
        ALTER TABLE public.matches 
        ADD CONSTRAINT fk_matches_expert_listing_id 
        FOREIGN KEY (expert_listing_id) REFERENCES public.expert_listings(id) ON DELETE CASCADE;
    END IF;
END $$;
