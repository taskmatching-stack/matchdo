...
DROP FUNCTION IF EXISTS mark_notifications_as_read CASCADE;
...
CREATE TABLE IF NOT EXISTS public.notifications (
    ...
    type text NOT NULL CHECK (type IN (
        ...
        'credits_granted',          -- 點數贈送（訂閱方案每月附贈）
        'credits_consumed',         -- 點數消費（AI生圖/拆包等）
        'credits_low',              -- 點數不足提醒
        
        -- 廠商列表相關（長期訂閱專屬）
        'vendor_profile_activated', -- 廠商公開資料已啟用
        'vendor_profile_viewed',    -- 廠商資料被查看
        'vendor_featured',          -- 成為精選廠商
        'long_term_subscription_benefit', -- 長期訂閱權益提醒
...
DO $$
BEGIN
    -- 加入 related_match_id 外鍵
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_schema = 'public' AND table_name = 'matches'
    ) THEN
        ALTER TABLE public.notifications 
        ADD CONSTRAINT fk_notifications_match_id 
        FOREIGN KEY (related_match_id) REFERENCES public.matches(id) ON DELETE CASCADE;
    END IF;
    
    -- 加入 related_unlock_id 外鍵
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_schema = 'public' AND table_name = 'contact_unlocks'
    ) THEN
        ALTER TABLE public.notifications 
        ADD CONSTRAINT fk_notifications_unlock_id 
        FOREIGN KEY (related_unlock_id) REFERENCES public.contact_unlocks(id) ON DELETE CASCADE;
    END IF;
    
    -- 加入 related_project_id 外鍵
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
    ...
    -- 加入 related_listing_id 外鍵
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_schema = 'public' AND table_name = 'expert_listings'
    ) THEN
        ALTER TABLE public.notifications 
        ADD CONSTRAINT fk_notifications_listing_id 
        FOREIGN KEY (related_listing_id) REFERENCES public.expert_listings(id) ON DELETE CASCADE;
    END IF;
END $$;
