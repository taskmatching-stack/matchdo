...
## 🗺️ 網站地圖
詳細路徑對照請參考: **[docs/sitemap.md](./sitemap.md)**
...
2. [ ] 建立 `client/project-detail.html`（優先度：高）**← 下一個**
   - 專案完整資訊
...
3. [ ] 建立 `client/project-items.html`（分包項目管理）（優先度：高）
...
4. [ ] 檢查 `client/my-projects.html`（優先度：中）
   - 確認能正確載入專案列表
   - **顯示專案發包狀態**
     - 顯示統包組數量（X 個統包組）
     - 顯示單獨分項數量（Y 個單項）
...
- ✅ Phase 1.4 - 專家功能（100%）
  ...
  - 報價列表與表單 ✅
  - 媒合專案頁面 ✅（UI 完成，待接真實資料）
- ⏳ Phase 1.5 - 發案者功能（30% → **50%** 更新中）
  - ✅ 我的專案列表（已有頁面，需檢查功能）
  - ✅ 客製產品相關頁面
  - ✅ **發案者控制台（2026-02-05 完成）** ⭐
  - ⏳ 專案詳情頁（待建立）**← 下一個**
  - ⏳ 媒合專家頁面（待建立）
  - **依據新路線圖：階段一資料庫已完成 ✅**
