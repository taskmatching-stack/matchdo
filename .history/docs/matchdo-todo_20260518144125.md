...
| **本檔 §4「產業供應商、製造商材料庫」** | 製造商後台**收藏即導入**產業原料、**我的材料庫**（B2B 採購用）；**不**接入訂製者設計頁／AI 試穿；另含線下店家目錄、官方虛擬廠商；**暫不實作**。 |
...
### 步驟 4.5：廠商素材庫上傳介面（高優先，可同步或接步驟 4 執行）

**目標**：讓製造商上傳設計素材（參考圖、設計範本、材料樣本），訂製者在設計時可直接從廠商素材庫選取參考圖，提升設計體驗。

**4.5.1 SQL 建立表與 RLS**

在 Supabase SQL Editor 執行 `docs/create-vendor-assets-schema.sql`：
- 建立 `vendor_assets` 表：`id`、`manufacturer_id`、`asset_type`（reference_image/design_template/material_sample）、`category_key`、`name`、`description`、`image_url`、`tags`、`sort_order`、`is_active`、`created_at`、`updated_at`
- RLS policies：任何人可讀已啟用素材；製造商只能管理自己的素材
- 索引優化：`manufacturer_id`、`category_key`、`is_active`

**4.5.2 後端 API（`server.js` 新增 4 個端點）**

1. `GET /api/me/vendor-assets` → 廠商查看自己所有素材
2. `POST /api/me/vendor-assets` → 廠商上傳新素材（含 multipart file 處理、圖片上傳至 Supabase Storage、儲存元資料）
3. `DELETE /api/me/vendor-assets/:id` → 廠商刪除素材
4. `GET /api/vendor-assets/:manufacturerId/:categoryKey` → 訂製者查看某廠商特定分類的素材（無認證）

**4.5.3 廠商後台頁面（`expert/vendor-assets.html`）**

- 頁面標題列：「我的設計素材庫」
- 分類篩選選單（對應訂製品分類）
- 「上傳素材」按鈕 → 觸發 modal
- 素材 Grid 展示：縮圖、名稱、分類、標籤、編輯／刪除按鈕
- modal 表單：素材類型（單選）、對應分類（下拉）、名稱、描述、標籤（逗號分隔）、圖片上傳（預覽+拖放）
- 上傳後即時刷新列表

**4.5.4 訂製者設計頁整合（`custom-product.html` 修改）**

- 在分類選擇下方新增「從廠商素材庫選」按鈕（或放在參考圖上傳區旁）
- 點擊後展開該分類所有廠商的素材 modal（Grid 展示）
- 訂製者可預覽、點擊素材後將該圖新增至「參考圖」區域
- UI 簡潔，不要遮擋現有功能
...
- [ ] SQL 在 Supabase 已執行
- [ ] 四個 API 能正常呼叫、權限正確
- [ ] 廠商後台 `expert/vendor-assets.html` 可訪問、上傳／刪除素材無誤
- [ ] 訂製者在 `custom-product.html` 可點「從廠商素材庫選」、檢視素材、加入參考圖
- [ ] 無 500 報錯、權限檢查正確（廠商只看自己的、訂製者無認證也能看已啟用素材）

**4.5.6 優化建議（可稍後）**

- 素材排序（sort_order）與搜尋（標籤、名稱搜尋）
- 廠商素材「停用」而非刪除
- 訂製者「加入收藏」素材為快速參考
...
| 🔴 4.5 | **廠商素材庫上傳介面**（高優先） | 步驟 4.5（SQL + 三個 API + 廠商後台頁面 + 訂製者設計頁整合） |
| 5 | 後排與可選（廠商搜尋、project-detail UI、金流、通知、README） | 步驟 5 |
| 6 | **手機／平板 App 轉化**（可選，見 `docs/app-mobile-tablet-plan.md`） | 步驟 6 |
| 後排 | **產業供應商數位材料庫（收藏即導入）** | 見 **§4.3～§4.5**；僅製造商後台採購用，與訂製者 `custom-product` 無關 |
...
| 2 | **再製方案 E2E**（可選） | 步驟 2 | — |
| 2.5 | **廠商素材庫上傳介面**（高優先） | 步驟 4.5 | ✅ 可同步或接步驟 4 執行 |
| 3 | **上雲與環境** | 步驟 3 | — |
...
### 4. 產業供應商、製造商材料庫與相關規劃（討論用，暫不實作）

以下整合：**線下店家目錄**、**官方虛擬廠商**、**產業供應商數位材料庫**、**收藏即導入（製造商快速取得原料）**。實作前請依此節與需求方確認。  
**帳號原則**：不新增「訂製者／製造商」身分切換；僅 **admin** 特殊。其餘以 **資料資格（capabilities）** 控制功能是否開放。  
**範圍澄清（2026-05-12）**：產業供應商材料 = **製造商內部採購／備料參考**，與**訂製者**在產品設計頁的參考圖、收藏、AI 試穿／實境模擬**無關**；亦**不**透過 `?manufacturer_id=` 載入。
...
#### 4.3 產業供應商與數位材料庫（B2B；製造商專用）

**定位**：上游**產業供應商**維護**數位材料／原料品項**；**製造商**（已建廠商且至少 1 筆 `manufacturer_portfolio`）在**製造商後台**瀏覽並**收藏（即導入）**至「我的材料庫」，方便**快速查規格、聯絡進貨**，與報價／備料流程銜接。  
...
| 產業供應商材料 | 產業供應商／管理員 | 製造商（有作品門檻） | 🆕 `industry_suppliers` + `supplier_products` + 導入表 |
...
- 建議 `GET /api/me/capabilities` 回傳：`has_manufacturer`、`portfolio_count`、`can_use_supplier_catalog`、`can_import_supplier_products`。
...
| `supplier_products` | 數位材料／產品：主圖、名稱、規格摘要、分類、所屬供應商 |
| `manufacturer_supplier_imports`（或 `manufacturer_favorite_materials`） | **收藏即導入**：`manufacturer_id` + `supplier_product_id` + 導入時快照（名稱、圖、規格）+ `imported_at`、製造商備註（可選） |

**「導入」定義**：**收藏即導入** — 不整庫同步，只加入製造商自己的清單；預設**快照**，避免供應商改價／改圖造成糾紛。

#### 4.4 製造商後台（Maker Dashboard）必做行為

| 行為 | 說明 |
|------|------|
| **⭐ 加入我的材料庫** | 製造商在**產業供應商目錄**瀏覽原料／材料時，點擊即**收藏並導入**（同一動作）。`POST /api/me/supplier-product-imports`，body：`supplier_product_id`。 |
| **我的材料庫** | **製造商專用頁**（例：控制台入口「我的材料庫」或 `/client/manufacturer-sourced-materials.html`）：已導入列表、依分類／供應商篩選、備註、一鍵聯絡供應商、取消導入。 |
| **快速取得原料** | 目的 = 報價／打樣／備料時快速找到曾導入的品項與規格，**不是**給終端使用者選設計參考。 |
| **解鎖提示** | 無作品時：顯示「上傳至少 1 件作品後可使用產業供應商材料庫」。 |
| **上傳第 1 件作品後** | 可 toast：「已解鎖產業供應商材料庫」。 |

#### 4.5 與訂製者設計流分離（必讀；先前規劃已修正）

| 項目 | 說明 |
|------|------|
| **不做** | 已導入的產業供應商材料**不**出現在 `custom-product.html`、**不**接入 AI 試穿／實境模擬（`scene-sim`）、**不**因 `?manufacturer_id=` 自動載入、**不**進訂製者「我的數位資產」或設計參考圖流程。 |
| **`?manufacturer_id=` 維持現狀** | 僅服務**訂製者**從廠商頁「用此廠商版型設計」→ 載入該製造商的 **`vendor_assets`（對外版型）**，與產業原料庫無關。 |
| **兩條線對照** | ① 訂製者設計線：`vendor_assets` → 產品設計頁。② 製造商採購線：`supplier_products` → 收藏導入 → 我的材料庫（後台）。 |

#### 4.6 介面文案（僅製造商後台，可選）

| 項目 | 說明 |
|------|------|
| **可選標題** | 製造商後台「我的材料庫」區塊可用 **「本店常用原料」** 等內部用語，方便辨識已導入品項；**僅製造商本人可見**，不對訂製者展示。 |
| **顯示供應商名** | 後台列表建議顯示**真實產業供應商名稱**與聯絡方式，方便採購。 |
| **進階（第二階段，另議）** | 若製造商要把**某張圖**給訂製者當設計參考，應走既有 **`vendor_assets` 上傳**流程，並自行處理授權；**不是**把「導入的產業原料」自動同步到設計頁。 |

#### 4.7 API 清單（概念，實作時對照）

| API | 誰 | 說明 |
|-----|-----|------|
| `GET /api/me/capabilities` | 登入者 | 含供應商目錄／收藏資格 |
| `GET /api/industry-suppliers` | 有資格製造商 | 供應商列表 |
| `GET /api/industry-suppliers/:id/products` | 有資格 | 該供應商數位材料 |
| `POST /api/me/supplier-product-imports` | 有資格 | **收藏即導入** |
| `GET /api/me/supplier-product-imports` | 有資格 | 我的材料庫 |
| `DELETE /api/me/supplier-product-imports/:id` | 有資格 | 取消收藏 |
...
| 階段 | 內容 |
|------|------|
| **P0** | SQL：`industry_suppliers`、`supplier_products`、`manufacturer_supplier_imports`；capabilities；管理員上架供應商與產品 |
| **P1** | 製造商後台：產業供應商目錄、**⭐ 加入我的材料庫**、「我的材料庫」列表與聯絡 |
| **P2** | 篩選、備註、匯出／複製規格（可選）；入口與解鎖提示打磨 |
| **P3** | 供應商自助／月費名錄 |
| **並行可選** | §4.1 線下店家目錄、§4.2 官方虛擬廠商（與本節分開排程） |

#### 4.9 風險與規則

- 產業供應商上架即同意：已解鎖之製造商可於平台內瀏覽與收藏參考；禁止批量爬取對外轉售。  
- 聯絡方式僅有資格製造商可見；供應商列表／材料 API 不進公開 SEO。  
- 訂製者**看不到**產業供應商目錄與已導入原料；訂製者設計仍只用 `vendor_assets`。  
- 與 `vendor_assets`、種子廠商、線下 Maps 目錄：**分表、分入口**，避免一個 `manufacturers` 承載全部角色。

**需求一句話**：產業供應商維護數位原料品項；製造商（有作品）在後台**收藏即導入**至「我的材料庫」以快速備料與聯絡進貨；**與訂製者設計頁、AI 試穿、`manufacturer_id` 參數無關**。
