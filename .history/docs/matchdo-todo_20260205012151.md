...
- [ ] 在 Supabase Dashboard 啟用 Google OAuth Provider
- [ ] 建立 `public/config/auth-config.js`（Supabase Auth SDK 初始化）
- [ ] 建立 `public/login.html`（Google 登入頁面）
- [ ] 建立 `public/register.html`（註冊時選擇角色）
- [ ] 建立 `public/js/auth-middleware.js`（導航保護邏輯）
- [ ] 修改所有頁面載入時檢查登入狀態

### Phase 1.2 - 資料表設計（Week 1）
- [x] 導入 Supabase 專案,建立 `.env` 並接 Supabase JS SDK（已完成連線）。
- [ ] 建立 `users` 表（role: client/expert, profile_completed）
- [ ] 建立 `experts_profile` 表（專長、評分、作品集、認證狀態）
- [ ] 建立 `listings` 表（專家上架的報價項目，含 AI Tags）
- [ ] 修改 `projects` 表（新增 owner_id, status 欄位）
- [ ] 建立 RLS policies（用戶只能存取自己資料）
