...
#### A. 發案者控制台（client/dashboard.html）⭐⭐⭐
**目的**：讓發案者有自己的控制面板
- [ ] 統計卡片（我的專案數、待處理媒合、已完成項目）
- [ ] 最新專案列表
- [ ] 快速操作按鈕（新增專案、新增客製產品）
- [ ] 最新媒合專家列表
...
#### B. 媒合通知系統 ⭐⭐⭐
**目的**：讓雙方知道有新的媒合
- [ ] 建立 `notifications` 表（通知類型、內容、已讀狀態）
- [ ] 導航欄通知圖示（未讀數量提示）
- [ ] 通知列表頁面
- [ ] Email 通知整合（可選）

#### C. 聯絡資訊解鎖流程 ⭐⭐⭐
**目的**：實現「媒合成功才能看聯絡方式」的核心功能
- [ ] 在專案詳情頁顯示「申請查看聯絡方式」按鈕
- [ ] 建立 contact_unlocks 記錄（pending 狀態）
- [ ] 對方收到通知並可接受/拒絕
- [ ] 接受後雙方可查看對方聯絡資訊
- [ ] 使用 unlocked_contacts 檢視顯示已解鎖資訊

#### D. 真實媒合邏輯實作 ⭐⭐
**目的**：目前是 mock 資料，需要實作真實 AI 媒合
- [ ] 建立 `matches` 表（專案 <-> 專家報價的媒合記錄）
- [ ] 實作 AI 媒合算法（比對專案需求與報價項目）
- [ ] 計算媒合分數（關鍵字、預算、地區、時程）
- [ ] 在 matched-projects.html 顯示真實媒合結果
- [ ] 在 client 側建立對應的「媒合專家」頁面

### 優先級 2：體驗優化（建議下週完成）

#### E. 專案詳情頁面 ⭐⭐
- [ ] client/project-detail.html（發案者查看自己的專案）
- [ ] expert/project-detail.html（專家查看媒合到的專案）
- [ ] 顯示 AI 分析結果
- [ ] 顯示媒合的專家/報價列表
- [ ] 聯繫功能整合

#### F. 報價詳情頁面 ⭐⭐
- [ ] expert/listing-detail.html（專家查看自己的報價）
- [ ] 顯示報價完整資訊
- [ ] 顯示媒合到的專案列表
- [ ] 編輯/下架功能

#### G. 客製產品完整流程 ⭐
**目前狀態**：已有頁面和 API，但缺少資料表
- [ ] 執行 custom_products、manufacturers 表建立腳本
- [ ] 實作真實廠商媒合（目前是 mock）
- [ ] 建立廠商管理後台
- [ ] 整合聯繫功能
...
**推薦方案：Vercel（全端部署）**
- ✅ 支援 Node.js 後端（Serverless Functions）
- ✅ 自動從 GitHub 部署
- ✅ 免費額度充足（個人專案）
- ✅ 內建 CI/CD
- ✅ 自動 HTTPS
- ⚠️ 需要調整：將 Express routes 改為 Serverless Functions

**替代方案 1：Railway**
- ✅ 完整支援 Node.js 應用
- ✅ 不需修改程式碼架構
- ✅ 自動從 GitHub 部署
- ✅ 內建資料庫（但我們用 Supabase，不需要）
- 💰 免費額度：$5/月
...
**不推薦：Netlify**
- ❌ 主要用於靜態網站
- ❌ Functions 有限制（不適合複雜後端）
- ⚠️ 可以前端用 Netlify，後端用其他服務（但管理麻煩）
