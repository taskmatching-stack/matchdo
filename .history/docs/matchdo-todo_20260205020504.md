...
### A. 資料層（本週）
- [ ] 建立 `custom_products` 資料表（欄位：id、owner_id、title、description、reference_image_url、ai_image_url、analysis_json、status、created_at）。
- [ ] 建立 `manufacturers` 資料表（欄位：id、name、categories、location、rating、contact_json、created_at）。
- [ ] 建立對應 RLS policies（owner_id 可讀寫；公開欄位可讀）。

### B. API 層（本週）
- [ ] `POST /api/custom-products`：儲存客製需求（文字/示意圖）
- [ ] `GET /api/custom-products`：依登入者列出歷史紀錄
- [ ] `POST /api/custom-products/:id/match`：觸發媒合並儲存結果
- [ ] `POST /api/custom-products/:id/contact`：建立「聯繫廠商」流程（先記錄事件）

### C. 前端（下週）
- [ ] 客製產品歷史記錄頁面（列表 + 詳細）
- [ ] 媒合結果呈現（依分數排序 + 一鍵聯繫）
- [ ] AI 圖片生成錯誤處理 UI（重試、備援提示）
