...
DROP VIEW IF EXISTS public.project_items_summary CASCADE;
DROP VIEW IF EXISTS public.package_groups_detail CASCADE;
DROP FUNCTION IF EXISTS create_package_group CASCADE;
DROP FUNCTION IF EXISTS ungroup_package CASCADE;
DROP FUNCTION IF EXISTS publish_project_items CASCADE;
DROP FUNCTION IF EXISTS update_project_items_updated_at CASCADE;

-- 嘗試移除和新增 projects 表的統包欄位（如果表存在）
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'projects') THEN
        ...
        DROP COLUMN IF EXISTS published_items;
        ...
        EXECUTE 'COMMENT ON COLUMN public.projects.project_type IS ''專案類型：turnkey=統包, split=分包''';
        EXECUTE 'COMMENT ON COLUMN public.projects.total_items IS ''分包項目總數''';
        EXECUTE 'COMMENT ON COLUMN public.projects.published_items IS ''已發包項目數''';
    END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS public.project_items (
    ...
    project_id uuid NOT NULL,  -- 暫時移除外鍵
    item_name text NOT NULL,
    item_description text,
    category_id uuid,  -- 暫時移除外鍵
    category_name text,
    budget_min integer,
    budget_max integer,
...
BEGIN
    ...
    IF EXISTS (
        ...
        ADD CONSTRAINT fk_project_items_project_id 
        FOREIGN KEY (project_id) REFERENCES public.projects(id) ON DELETE CASCADE;
    END IF;
    
    -- 加入 category_id 外鍵（檢查表和 id 欄位是否都存在）
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_schema = 'public' AND table_name = 'ai_categories'
    ) AND EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'public' AND table_name = 'ai_categories' AND column_name = 'id'
    ) THEN
        ALTER TABLE public.project_items 
        ADD CONSTRAINT fk_project_items_category_id 
        FOREIGN KEY (category_id) REFERENCES public.ai_categories(id);
    END IF;
END $$;
