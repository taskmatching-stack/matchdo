...
app.get('/api/categories', async (req, res) => {
    try {
        // 1. 優先嘗試讀取正規化後的新表 (main_categories + sub_categories)
        const { data: mains, error: mainError } = await supabase
            .from('main_categories')
            .select('*, sub_categories(*)');
        
        if (!mainError && mains && mains.length > 0) {
            const categories = mains.map(m => ({
                key: m.key,
                name: m.name,
                prompt: m.prompt,
                sub: m.sub_categories.map(s => s.name),
                sub_configs: m.sub_categories.reduce((acc, s) => {
                    acc[s.name] = s.form_config;
                    return acc;
                }, {})
            }));
            res.set('Cache-Control', 'no-store');
            return res.json({ categories, via: 'normalized-db' });
        }

        // 2. 若新表無資料，回退至舊的 ai_categories 表
        const { data, error } = await supabase
            .from('ai_categories')
            .select('key, name, prompt, subcategories, sub_configs');
        if (error) {
...
app.put('/api/categories', express.json(), async (req, res) => {
    try {
        ...
        for (const cat of categories) {
            ...
            if (mainError) {
                // 如果新表不存在，直接跳到步驟 3
                console.warn('New tables might not exist yet, skipping normalized update.');
            } else {
                // 2. 更新子分類（新表）
                await supabase.from('sub_categories').delete().eq('main_category_id', mainCat.id);
                if (cat.sub && Array.isArray(cat.sub)) {
                    const subPayload = cat.sub.map(subName => ({
                        main_category_id: mainCat.id,
                        name: subName,
                        form_config: (cat.sub_configs && cat.sub_configs[subName]) ? cat.sub_configs[subName] : []
                    }));
                    await supabase.from('sub_categories').insert(subPayload);
                }
            }

            // 3. 保持舊表同步（確保系統運作）
            await supabase.from('ai_categories').upsert({
                key: cat.key,
                name: cat.name,
                prompt: cat.prompt || '',
                subcategories: cat.sub || [],
                sub_configs: cat.sub_configs || {}
            });
        }

        res.json({ success: true, message: '分類資料已儲存' });
    } catch (e) {
