    ...
    async function renderCategoryOptions() {
        ...
        try {
            ...
            const list = Array.isArray(data.categories) ? data.categories : (window.AI_CATEGORIES || []);
            window.__AI_CATEGORIES_CACHE__ = list;
            list.forEach(function (cat) {
                $cat.append(`<option value="${cat.key}">${cat.name}</option>`);
            });
        } catch (e) {
            (window.AI_CATEGORIES || []).forEach(function (cat) {
                $cat.append(`<option value="${cat.key}">${cat.name}</option>`);
            });
        }
        $cat.trigger('change');
    ...
    function renderSubcategoryOptions(catKey) {
        var $sub = $('#subcategory');
        $sub.empty();
        var src = window.__AI_CATEGORIES_CACHE__ || window.AI_CATEGORIES || [];
...
(function ($) {
    ...
    $('#aiForm').on('submit', function (e) {
        ...
        // 將多選子分類合併進 formData
        var subcats = $('#subcategory').val();
        if (subcats && subcats.length > 0) formData.append('subcategories', JSON.stringify(subcats));
        ...
        $.ajax({
            ...
            success: function (res) {
                let headerInfo = '';
                if (res.project_id) {
                    headerInfo = `<div class="alert alert-success">已建立專案記錄：ID ${res.project_id}。你可在 Supabase 的 projects 表查看。</div>`;
                }
