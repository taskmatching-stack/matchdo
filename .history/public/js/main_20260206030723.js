(function ($) {
    ...
    function updateCategoryHint() {
        const selectedSubcats = $('#subcategory').val() || [];
        const $hint = $('#categoryHint');
        const $dynamicContainer = $('#dynamicFieldsContainer');
        const currentCatKey = $('#category').val();
        
        $dynamicContainer.empty();

        const src = window.__AI_CATEGORIES_CACHE__ || [];
        const cat = src.find(c => c.key === currentCatKey);
        
        if (!cat) return;

        // 1. 產生動態填寫欄位 (內建每個分類 3 個預設欄位)
        const defaultConfigs = {
            'home': {
                '水電工程': [
                    {"label": "維修位置", "name": "location", "type": "text", "placeholder": "例如：廚房流理台"},
                    {"label": "漏水狀況", "name": "leakage", "type": "select", "options": ["無", "輕微滲漏", "嚴重漏水", "管路破裂"]},
                    {"label": "插座/開關數量", "name": "sockets", "type": "number", "unit": "個"}
                ],
                '清潔服務': [
                    {"label": "施作坪數", "name": "area", "type": "number", "unit": "坪"},
                    {"label": "清潔類型", "name": "clean_type", "type": "select", "options": ["日常清潔", "空屋細清", "裝潢後粗清"]},
                    {"label": "是否有電梯", "name": "elevator", "type": "select", "options": ["是", "否"]}
                ],
                '室內設計與裝潢': [
                    {"label": "房屋類型", "name": "house_type", "type": "select", "options": ["新成屋", "中古屋", "老屋翻新"]},
                    {"label": "室內坪數", "name": "interior_area", "type": "number", "unit": "坪"},
                    {"label": "格局需求", "name": "rooms", "type": "text", "placeholder": "例如：3房2廳"}
                ]
            },
            'event': {
                '場地 餐飲': [
                    {"label": "活動人數", "name": "guests", "type": "number", "unit": "人"},
                    {"label": "餐飲形式", "name": "food_style", "type": "select", "options": ["自助餐", "中式桌菜", "西式排餐", "茶點"]},
                    {"label": "預算區間", "name": "budget", "type": "text", "placeholder": "每人預算"}
                ]
            },
            'learn': {
                '各科家教': [
                    {"label": "科目名稱", "name": "subject", "type": "text", "placeholder": "例如：國中數學"},
                    {"label": "學員程度", "name": "level", "type": "select", "options": ["基礎", "進階", "考試衝刺"]},
                    {"label": "上課頻率", "name": "frequency", "type": "text", "placeholder": "每週幾次"}
                ]
            },
            'health': {
                '健身雕塑': [
                    {"label": "健身目標", "name": "goal", "type": "select", "options": ["減脂", "增肌", "體態校正"]},
                    {"label": "目前體重", "name": "weight", "type": "number", "unit": "kg"},
                    {"label": "運動經驗", "name": "experience", "type": "select", "options": ["新手", "一般", "運動員"]}
                ]
            },
            'business': {
                '平面設計': [
                    {"label": "設計項目", "name": "design_item", "type": "select", "options": ["Logo", "名片", "海報", "手冊"]},
                    {"label": "風格偏好", "name": "style", "type": "text", "placeholder": "例如：簡約、科技、華麗"},
                    {"label": "交付日期", "name": "deadline", "type": "date"}
                ]
            },
            'beauty': {
                '美容美髮': [
                    {"label": "服務項目", "name": "service", "type": "select", "options": ["剪髮", "染燙", "護髮", "造型"]},
                    {"label": "目前髮長", "name": "hair_length", "type": "select", "options": ["極短", "肩上", "過肩", "及腰"]},
                    {"label": "預約時段", "name": "timeslot", "type": "text", "placeholder": "期望的日期與時間"}
                ]
            }
        };

        const subConfigs = cat.sub_configs && Object.keys(cat.sub_configs).length > 0 
                           ? cat.sub_configs 
                           : (defaultConfigs[currentCatKey] || {});

        let fieldHtml = '';
        
        selectedSubcats.forEach(sub => {
            const fields = subConfigs[sub] || [];
            if (fields.length > 0) {
                fieldHtml += `<div class="col-12"><h6 class="text-secondary border-bottom pb-1 mt-2 mb-2"><i class="fas fa-edit me-2"></i>${sub} 專屬需求</h6></div>`;
                fields.forEach(f => {
                    fieldHtml += `<div class="col-md-6 mb-2">
                        <label class="form-label small mb-1">${f.label}${f.unit ? ` (${f.unit})` : ''}</label>`;
                    
                    if (f.type === 'select') {
                        fieldHtml += `<select class="form-select form-select-sm dynamic-field" data-sub="${sub}" data-label="${f.label}">
                            ${(f.options || []).map(opt => `<option value="${opt}">${opt}</option>`).join('')}
                        </select>`;
                    } else {
                        fieldHtml += `<input type="${f.type || 'text'}" class="form-control form-control-sm dynamic-field" 
                            data-sub="${sub}" data-label="${f.label}" placeholder="${f.placeholder || ''}">`;
                    }
                    fieldHtml += `</div>`;
                });
            }
        ...
        if (fieldHtml) $dynamicContainer.html(fieldHtml);

        const hints = {
            '水電工程': '請註明：漏水位置、需更換插座數量等。',
            '清潔服務': '請註明：期望完工日期等。'
        };

        let activeHints = [];
        selectedSubcats.forEach(sub => {
            if (hints[sub]) activeHints.push(`【${sub}】${hints[sub]}`);
        });

        if (activeHints.length > 0) {
            $hint.html('<i class="fas fa-lightbulb me-1"></i>備註引導：<br>' + activeHints.join('<br>'));
        } else {
            $hint.text('');
        }
    }
    ...
    $('#aiForm').on('submit', function (e) {
        ...
        // 強制檢查：主分類為「居家」時，必須選擇子分類
        if (categoryText === '居家' && subcats.length === 0) {
            alert('「居家」分類請至少選擇一個子分類，以確保 AI 辨識精準度。');
            $('#subcategory').focus();
            return;
        }
