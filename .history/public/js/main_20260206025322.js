(function ($) {
    ...
    function updateCategoryHint() {
        // ...existing code...
        // 1. 產生動態填寫欄位
        const subConfigs = cat.sub_configs || {
            '水電工程': [
                {"label": "維修位置", "name": "location", "type": "text", "placeholder": "例如：廚房流理台下方"},
                {"label": "漏水情況", "name": "leak_desc", "type": "text", "placeholder": "請描述嚴重程度"}
            ],
            '清潔服務': [
                {"label": "施作坪數", "name": "area", "type": "number", "unit": "坪"},
                {"label": "清潔類型", "name": "type", "type": "select", "options": ["空屋清潔", "居家日常", "裝潢完工後細清"]}
            ],
            '室內設計與裝潢': [
                {"label": "房屋類型", "name": "house_type", "type": "select", "options": ["新成屋", "中古屋", "老屋翻新"]},
                {"label": "施作空間", "name": "space", "type": "text", "placeholder": "例如：客廳、全室"}
            ]
        };
        let fieldHtml = '';
        // ...existing code...
        ...
        if (fieldHtml) {
            $dynamicContainer.html(fieldHtml);
        }
