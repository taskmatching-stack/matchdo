(function ($) {
    ...
    function updateCategoryHint() {
        ...
        const $input = $('#projectDescriptionInput');
        
        const hints = {
            '水電工程': '請註明：漏水位置、需更換插座數量、水壓狀況或電線抽換範圍。',
            '清潔服務': '請註明：施作坪數、空屋或居家清潔、是否包含窗戶/冷氣外殼、期望完工日期。',
            '室內設計與裝潢': '請註明：房屋類型（新成屋/老屋）、施作空間（客廳/全室）、喜好風格及預算範圍。',
            '廚房': '請註明：是否需拆除舊廚具、三機品牌需求、櫥櫃材質（不鏽鋼/美耐板）。',
            '衛浴': '請註明：是否需做防水工程、磁磚款式需求、是否更換衛浴設備（馬桶/浴缸）。',
            '木工 油漆 壁紙': '請註明：天花板高度、隔間材質、油漆品牌及色號、壁紙範圍。'
        };
        ...
        if (activeHints.length > 0) {
            $hint.html('<i class="fas fa-lightbulb me-1"></i>填寫指引：<br>' + activeHints.join('<br>'));
        } else {
            $hint.text('');
        }
    }
