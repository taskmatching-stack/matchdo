    ...
    try {
        ...
        try {
            const { data: projectInserted, error: insertError } = await supabase
              .from('projects')
              .insert({ title, description: JSON.stringify({ prompt, files: filesInfo }), status: 'analyzing' }) // Use analyzing as draft
              .select('id')
              .limit(1);
...
app.post('/api/projects/update-items', async (req, res) => {
    try {
        ...
        const { error: updateError } = await supabase
            .from('projects')
            .update({ analysis: newAnalysis, status: 'analyzing' }) // 確保狀態為 analyzing (draft)
            .eq('id', project_id);
