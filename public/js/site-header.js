...
(function () {
    ...
    if (el && !el.innerHTML) {
        ...
        if (!document.getElementById('navbar-logo-center-style')) {
            ...
            s.textContent = '@media(min-width:992px){.navbar-brand{position:absolute;left:50%;transform:translateX(-50%);border-right:none!important;border-left:none!important;padding-right:0!important;padding-left:0!important;}}';
            document.head.appendChild(s);
        }
        el.innerHTML = '<nav class="navbar navbar-expand-lg bg-white navbar-light sticky-top p-0 position-relative">' +
            '<a href="/index.html" class="navbar-brand d-flex align-items-center px-4 px-lg-5"><img src="/img/matchdo-logo.png" alt="MatchDO 合做" style="height:52px;width:auto;"></a>' +
            ...
            '</div>' +
            (!isAdmin ? '<div class="d-none d-lg-flex align-items-center px-2 border-end"><a href="#" class="lang-link small text-muted text-decoration-none me-1" data-lang="zh-TW">中文</a><span class="text-muted">|</span><a href="#" class="lang-link small text-muted text-decoration-none ms-1" data-lang="en">EN</a></div>' : '') +
            '<div class="d-none d-lg-flex align-items-center px-4" id="authSection"><a href="' + loginHref + '" class="btn btn-primary py-2 px-4"><i class="bi bi-person me-2"></i>登入</a></div>' +
            '<div class="d-lg-none px-4 pb-3 pt-2 border-top mt-2" id="authSectionMobile"><a href="' + loginHref + '" class="btn btn-primary w-100 py-2"><i class="bi bi-person me-2"></i>登入</a></div>' +
            '</div></nav>';
    }
})();
...
async function renderHeader(headerContainer, user, config) {
    ...
    // ── 組出 auth / myFeatures HTML（供完整渲染與快速更新共用）──
    ...
    if (headerContainer.querySelector('#navbarCollapse')) {
        ...
        if (authSecMobile) {
            authSecMobile.innerHTML = (showLangSwitch ? '<div class="mb-2"><a href="#" class="lang-link small text-muted me-2" data-lang="zh-TW">中文</a><a href="#" class="lang-link small text-muted" data-lang="en">EN</a></div>' : '') + mobileAuthHTML;
        }
