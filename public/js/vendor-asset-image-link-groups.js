...
(function (global) {
    ...
    function defaultLinkedSelection(imageItems, maxSelect, labelForItem) {
        ...
        if (!imageItems || !imageItems.length) return [];

        var firstGroup = '';
        for (var i = 0; i < imageItems.length; i++) {
            var g = normalizeLinkGroup(imageItems[i].link_group);
        ...
        if (!pick.length) {
            var take = Math.min(maxSelect, imageItems.length);
            for (var j = 0; j < take; j++) {
                var it2 = imageItems[j];
                pick.push({
                    url: it2.url,
