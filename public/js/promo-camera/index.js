...
(function () {
  ...
  function syncPortraitStylingUi() {
    ...
    if (stylingHint) {
      if (mode === 'scene') {
        stylingHint.textContent = t('promoCamera.portraitStylingHintScene', '衣著由拍攝主題與場景決定；構圖預設商用、避免情色感；描述可寫姿勢、表情等。');
      } else if (mode === 'prompt') {
        stylingHint.textContent = t('promoCamera.portraitStylingHintPrompt', '服裝、髮型等請寫在描述欄（必填）；構圖預設商用、避免情色感。');
      } else {
