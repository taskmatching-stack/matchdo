...
(function () {
    ...
    function applyCatalogSelectOptions(lang) {
        lang = lang || currentLang();
        ['#bsCatalogMain option', '#bsCatalogSub option', '#bsCatalogAssetKind option'].forEach(function (sel) {
            document.querySelectorAll(sel).forEach(function (opt) {
                var zh = opt.getAttribute('data-name-zh');
                var en = opt.getAttribute('data-name-en');
                if (zh == null && en == null) return;
                opt.textContent = pickCatLabel(zh || '', en || '', lang, opt.getAttribute('data-cat-key') || '');
            });
        });
    }
    ...
    function applyBrowseCategoryI18n() {
        ...
        applyCatalogSelectOptions(lang);
